...
# Role decorators defined early so all endpoints can use them consistently.
# is_admin()/is_manager()/jsonify are resolved dynamically at request time.
def require_admin(f):
    @wraps(f)
    def _w(*a, **k):
        if not is_admin():
...
# Load from .env files if env vars are missing (local dev — .env* is gitignored)
if not SUPABASE_URL or not SUPABASE_KEY:
    _root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    for _fname in [".env.local", ".env", ".env.production"]:
        _env_path = os.path.join(_root_dir, _fname)
    ...
    SUPABASE_KEY = SUPABASE_KEY or os.environ.get("SUPABASE_KEY", "").strip()
    SECRET_KEY = os.environ.get("SECRET_KEY", "").strip()
    APP_SECRET = os.environ.get("APP_SECRET", "").strip()

if not (SUPABASE_URL and len(SUPABASE_URL) > 15 and SUPABASE_URL.startswith("http")):
    import warnings as _w
    _w.warn("SUPABASE_URL env var is missing or invalid — set it in Vercel/environment or .env file.")
    SUPABASE_URL = ""
if not (SUPABASE_KEY and len(SUPABASE_KEY) > 50):
    import warnings as _w2
    _w2.warn("SUPABASE_KEY env var is missing or invalid — set it in Vercel/environment or .env file.")
    SUPABASE_KEY = ""

HTML_B64 = ''
...
CANONICAL_DEFAULT_CLIENTS = [
    {
        "id": "client_100821894800009",
        "name": "Domya Marketing Agency",
        "company": "Domya Marketing Agency",
    ...
    {
        "id": "cli_dr_ahmed_fahmy_1788683119",
        "name": "DR AHMED FAHMY",
        "company": "DR AHMED FAHMY",
        "package": "Business VIP",
        "am_employee_id": "EMP-5887-5256",
        "am_name": "آيه أحمد مجاهد",
        "status": "active",
    ...
    {
        "id": "cli_hayat_dental_center_1788685057",
        "name": "HAYAT DENTAL CENTER",
        "company": "HAYAT DENTAL CENTER",
        "package": "Business VIP",
        "am_employee_id": "EMP-5887-5256",
        "am_name": "آيه أحمد مجاهد",
        "status": "active",
    ...
    {
        "id": "cli_dr_shimaa_atef_1788298157",
        "name": "Dr Shimaa Atef",
        "company": "Dr Shimaa Atef",
        "package": "Business VIP",
        "am_employee_id": "EMP-5887-5256",
        "am_name": "آيه أحمد مجاهد",
        "status": "active",
    ...
    {
        "id": "cli_dr_shahenda_1788685119",
        "name": "دكتورة شاهندة مختار",
        "company": "دكتورة شاهندة مختار",
        "package": "Business VIP",
        "am_employee_id": "EMP-5887-5256",
        "am_name": "آيه أحمد مجاهد",
        "status": "active",
...
CANONICAL_CLIENT_FOLDERS = {
    "client_100821894800009": "12Tr1P3DHfCHfN-uqSSymx65KCJqeN_TU",
    "cli_sk_1788270118": "1KV6-ImSXvlS5D8ryibB5bwyLBreGvmIW",
    "cli_dr_ahmed_1788270119": "1jbGNqWok5zqPuKUVnjhG5TrTecKyofCI",
    "cli_انفينيتي_1788270119": "1z1swoSgUhAJ1DIRdGa7Abm0DQ1ys85ye",
    "cli_معامل_رعاية_1788336726": "15N8jmo2KTVQnxwA2XPTZEakFXRDJNK9_",
    "cli_هبه_حافظ_1788431922": "1sRUc4QgIq66_QCDEvGbPr_VWlY-HGQng",
    "cli_dr_ahmed_fahmy_1788683119": "1H3FRpEjt-Ux8lYA0-NyFvKsBJ6uPWKzN",
    "cli_dr_hadeer_1788684282": "1kHWfwe3U-I5GUUPHSFps1QTyhohAHJcE",
    "cli_hayat_dental_center_1788685057": "19qiBi2bnK8NBK5WvFhRQY2jG9HlWYGNK",
    "cli_dr_shimaa_atef_1788298157": "1dshmjPJ81IgVA-1CrLvn2CRAC9YUvnoX",
    "cli_dr_shahenda_1788685119": "1BDLOScrrsSgBfLsiin1irzqzl4fFAMRC",
    "cli_ايه_عبدو_1788944266": "15GmrSkAoolWY9T1tz2ghrgKmZBEtGLrF"
}

CANONICAL_CLIENT_PARENT_FOLDERS = {
    "client_100821894800009": "1Yj8BD4uolg6OxjiGtQcLntuANbTie6Uf",
    "cli_sk_1788270118": "1X-UzI2662R1CEH1xnBln1RmVxQ9pY1Sz",
    "cli_dr_ahmed_1788270119": "1t-UAOtaRmRDJuiU_J3hgEzXzlyJkIi8y",
    "cli_انفينيتي_1788270119": "1EyamFhlzQ-2i60tlAJoE0zx2Rg_1SyHr",
    "cli_معامل_رعاية_1788336726": "1xc3IiM-tK3QtR0Tm-tQd7z9I4vk4LBsL",
    "cli_هبه_حافظ_1788431922": "1xHyIniLFZ70ebH07vvAXOHxJjUKjU2yn",
    "cli_dr_ahmed_fahmy_1788683119": "10NrGFGalISnfo_NXMyT09_FRZWPMSpur",
    "cli_dr_hadeer_1788684282": "1cvkpw04bPcHx_UFr40lvk6vnCdaBwO2x",
    "cli_hayat_dental_center_1788685057": "1fRbB9-xZVIpPbuWw424zSRpr0ikkcLBo",
    "cli_dr_shimaa_atef_1788298157": "1HBkbXPpfW0lZZ917jn4v0S2DdpTVwI0r",
    "cli_dr_shahenda_1788685119": "1SQceUfga50xZNvSKcjKFaCaus9xzEGwM",
    "cli_ايه_عبدو_1788944266": "1X5KCfURkWM_b4gEn8P83Jck1Nqj9n8AK"
}

CANONICAL_CLIENT_ALIASES = {
    "client_100821894800009": [
        "domya marketing agency", "domya marketing", "domya", "دمية", "وكالة دمية", "domya_marketing"
    ],
    "cli_dr_ahmed_1788270119": [
        "دكتور أحمد حمدي", "دكتور احمد حمدي", "د. أحمد حمدي", "د/ أحمد حمدي", "د. احمد حمدي", "د/ احمد حمدي",
        "أحمد حمدي", "احمد حمدي", "عيادة دكتور أحمد حمدي", "dr ahmed hamdy", "ahmed hamdy"
    ],
    "cli_sk_1788270118": [
        "sk", "sk clinic", "عيادة sk", "إس كي", "اس كي", "اس كى"
    ],
    "cli_انفينيتي_1788270119": [
        "انفينيتي", "إنفينيتي", "انفنيتي", "infinity", "عيادة انفينيتي", "مركز انفينيتي"
    ],
    "cli_معامل_رعاية_1788336726": [
        "معامل رعاية", "معمل رعاية", "مختبر رعاية", "رعاية", "reaya lab", "reaya", "rayah lab", "rayah"
    ],
    "cli_هبه_حافظ_1788431922": [
        "هبه حافظ", "هبة حافظ", "دكتورة هبه حافظ", "دكتورة هبة حافظ", "د. هبه حافظ", "د/ هبه حافظ",
        "heba hafez", "heba hafez clinic"
    ],
    "cli_dr_ahmed_fahmy_1788683119": [
        "dr ahmed fahmy", "ahmed fahmy", "fahmy", "دكتور أحمد فهمي", "دكتور احمد فهمي",
        "د. أحمد فهمي", "د/ أحمد فهمي", "د. احمد فهمي", "د/ احمد فهمي", "أحمد فهمي", "احمد فهمي",
        "عيادة دكتور أحمد فهمي", "fahmy content plan"
    ],
    "cli_dr_hadeer_1788684282": [
        "dr hadeer", "hadeer", "دكتورة هدير", "دكتور هدير", "د. هدير", "د/ هدير", "هدير",
        "عيادة دكتورة هدير"
    ],
    "cli_hayat_dental_center_1788685057": [
        "hayat dental center", "hayat dental", "hayat", "مركز حياة للأسنان", "مركز حياة",
        "حياة للأسنان", "عيادة حياة", "حياة دنتل سنتر", "حياة دنتال سنتر", "حياة"
    ],
    "cli_dr_shimaa_atef_1788298157": [
        "dr shimaa atef", "dr shymaa", "dr shimaa", "shimaa atef", "shymaa atef", "shimaa", "shymaa",
        "دكتورة شيماء عاطف", "دكتور شيماء عاطف", "د. شيماء عاطف", "د/ شيماء عاطف", "شيماء عاطف"
    ],
    "cli_dr_shahenda_1788685119": [
        "دكتورة شاهندة مختار", "دكتورة شاهندة", "دكتورة شاهنده", "د. شاهندة", "د. شاهنده",
        "د/ شاهندة", "د/ شاهنده", "دكتور شاهندة", "دكتور شاهنده", "شاهندة", "شاهنده", "شاهندا",
        "dr shahenda", "dr shahenda mokhtar", "shahenda", "shahenda mokhtar", "dr shahinda", "shahinda",
        "خطة dr shahenda", "خطة dr shahenda mokhtar", "خطة دكتورة شاهندة", "خطة دكتورة شاهنده"
    ],
    "cli_ايه_عبدو_1788944266": [
        "ايه عبدو", "آية عبدو", "ايه عبده", "آية عبده", "دكتورة ايه عبدو", "دكتورة آية عبدو",
        "aya abdo", "aya abdu", "خطة ايه عبدو", "خطة آية عبدو"
    ]
}
            ...
            if r.status_code == 200:
                ...
                for row in r.json():
                    ...
                    if k == "meta_ai_kb":
                        ...
                        clean_parsed = [c for c in parsed if isinstance(c, dict) and "إبداع" not in str(c.get("name","")) and "إبداع" not in str(c.get("company","")) and "عميل عام" not in str(c.get("name",""))]
                        existing_cids = {str(c.get("id")) for c in clean_parsed}
                        for cc in CANONICAL_DEFAULT_CLIENTS:
                            if cc["id"] not in existing_cids:
                                clean_parsed.append(dict(cc))
                        AGENCY_CLIENTS_STORE[:] = clean_parsed
                        cache["clients"] = clean_parsed
                ...
                try:
                    ...
                    print(f"[Redis set warning] {_rse}")
        except Exception as e:
...
def push_setting(key, value):
    # SAFETY: Never mutate remote database during automated tests
    if os.environ.get("PYTEST_CURRENT_TEST") or any("pytest" in str(arg).lower() for arg in sys.argv):
        return
    try:
    ...
    # SAFETY: never overwrite the accounts/clients lists with an empty value or truncated test list
    if key == "meta_ai_clients":
        if not value or not isinstance(value, list) or len(value) < 3:
            print(f"[Supabase Push BLOCKED] refusing to overwrite meta_ai_clients with truncated list ({len(value) if isinstance(value, list) else 0} items)")
            return
        # Ensure bogus / phantom / test clients never get pushed
        BOGUS_CLIENT_KEYWORDS = ("إبداع", "عميل عام", "نور", "النور", "creator_test", "تجرب", "test")
        value = [
            c for c in value
            if isinstance(c, dict)
            and not any(kw in str(c.get("name", "")).lower() or kw in str(c.get("id", "")).lower() or kw in str(c.get("company", "")).lower() for kw in BOGUS_CLIENT_KEYWORDS)
        ]
    if key == "meta_ai_accounts" and (not value or not isinstance(value, list)):
        print(f"[Supabase Push BLOCKED] refusing to overwrite {key} with empty value")
        return
    try:
        redis_del("app_settings_sync")
    except Exception as _rde:
        print(f"[Redis del warning] {_rde}")
    if SUPABASE_URL and SUPABASE_KEY:
        val_str = json.dumps(value, ensure_ascii=False) if isinstance(value, (dict, list)) else str(value)
        pushed = False
        # Try PATCH first for existing setting key (cleanest for large setting objects)
        patch_url = f"{SUPABASE_URL}/rest/v1/app_settings?key=eq.{key}"
        patch_headers = {
            "apikey": SUPABASE_KEY,
            "Authorization": f"Bearer {SUPABASE_KEY}",
            "Content-Type": "application/json",
        ...
        for attempt in range(3):
            try:
                res = requests.patch(patch_url, headers=patch_headers, json={"value": val_str}, timeout=25)
                if res.status_code in (200, 204):
                    print(f"[Supabase Patch OK] {key} -> HTTP {res.status_code}")
                    pushed = True
                    break
            except Exception as _pe:
                print(f"[Supabase Patch attempt {attempt+1} err] {key}: {_pe}")
                time.sleep(1)

        if not pushed:
            # Fallback to POST with merge-duplicates if key does not exist yet
            for attempt in range(2):
                try:
                    url = f"{SUPABASE_URL}/rest/v1/app_settings"
                    res = requests.post(url, headers=supa_headers(), json={"key": key, "value": val_str}, timeout=25)
                    if res.status_code in (200, 201, 204):
                        print(f"[Supabase Push OK] {key} -> HTTP {res.status_code}")
                        break
                except Exception as e:
                    print(f"[Supabase Push attempt {attempt+1} err] {key}: {e}")
                    time.sleep(1)

sync_from_supabase()

AGENCY_CLIENTS_STORE = cache.get("clients") or [dict(c) for c in CANONICAL_DEFAULT_CLIENTS]
...
def _call_groq(user_message, rag_context, platform="facebook", client_id=None, **kwargs):
...
def _call_openrouter(user_message, rag_context, platform="facebook", client_id=None, **kwargs):
    sender_id = kwargs.get("sender_id")
    system_prompt = get_system_prompt(client_id)
    if sender_id:
        system_prompt += get_customer_style_prompt(sender_id)
...
def log_event(event_type, sender, message, reply, **kwargs):
    private_reply = kwargs.get("private_reply")
    ...
    for k, v in USERS_DB.items():
        if isinstance(v, dict):
            if str(k).strip().lower() == u_str or str(v.get("employee_id") or "").strip().lower() == u_str:
                return v
    # Fallback lookup in KNOWN_EMPLOYEE_ROSTER
    if u_str in KNOWN_EMPLOYEE_ROSTER:
        eid, name = KNOWN_EMPLOYEE_ROSTER[u_str]
        role_info = KNOWN_EMPLOYEE_ROLES.get(u_str, ("content_creator" if u_str in CREATOR_EMPLOYEE_IDS else "employee", "Employee", ["myportal"]))
        return {
            "username": eid,
            "employee_id": eid,
            "name": name,
...
def _my_employee_id():
    ...
    if not eid:
        ...
        if uid.startswith("EMP-") or uid.startswith("AM-"):
            return uid
        u_low = uid.lower()
        if u_low in KNOWN_EMPLOYEE_ROSTER:
            return KNOWN_EMPLOYEE_ROSTER[u_low][0]
        if u_low in KNOWN_CREATOR_ALIASES:
            return KNOWN_CREATOR_ALIASES[u_low][0]
        u_norm = _norm_ar_str(uid)
        for al, (al_id, al_nm) in KNOWN_CREATOR_ALIASES.items():
            if _norm_ar_str(al) == u_norm or _norm_ar_str(al_nm) == u_norm:
                return al_id
...
def is_content_creator(role=None):
    if role is None:
        role = current_role()
    r_str = str(role or "").lower().strip()
    if r_str in ("content_creator", "content", "copywriter", "writer"):
        return True
    u_rec = current_user_rec()
    job = str(u_rec.get("job") or u_rec.get("role") or "").lower()
    if any(w in job for w in ("content", "creator", "writer", "كاتب", "محتوى", "creative")):
        return True
    my_eid = str(_my_employee_id() or "").strip().lower()
    if my_eid in CREATOR_EMPLOYEE_IDS or "2945" in my_eid or "8069" in my_eid or "7189" in my_eid or "3264" in my_eid or "7775" in my_eid:
        return True
    if has_request_context():
        uid = str(session.get("uid") or "").strip().lower()
        if uid in CREATOR_EMPLOYEE_IDS or "2945" in uid or "8069" in uid or "7189" in uid or uid in KNOWN_CREATOR_ALIASES:
            return True
        for al, (al_id, al_nm) in KNOWN_CREATOR_ALIASES.items():
            if _norm_ar_str(al) == _norm_ar_str(uid) or _norm_ar_str(al_nm) == _norm_ar_str(uid):
                return True
    if my_eid:
        try:
            cfg = hr_config()
            for e in _gsheet_rows(cfg["sheet_id"], cfg["employees_gid"]):
                if str(e.get("employee_id") or "").strip().lower() == my_eid:
                    ejob = str(e.get("job") or e.get("role") or "").lower()
                    if any(w in ejob for w in ("content", "creator", "writer", "كاتب", "محتوى")):
                        return True
        except Exception as _e:
...
def _user_session_token(username, role):
    ...
    if not secret:
        ...
        _ws.warn("SECRET_KEY env var not set — using random key (sessions won't persist across deploys).")
        if not hasattr(_user_session_token, '_rnd_secret'):
            _user_session_token._rnd_secret = os.urandom(32).hex()
        secret = _user_session_token._rnd_secret
            ...
            if not is_self_endpoint:
                ...
                for prefix, tab in _TAB_API_GUARD.items():
                    if p.startswith(prefix) and tab not in eff_tabs:
                        # Allow GET /api/clients and client switching for users with plan, tasks, or content creator privileges
                        if p.startswith("/api/clients") and (request.method == "GET" or p == "/api/clients/switch") and (is_content_creator() or "plan" in eff_tabs or "tasks" in eff_tabs):
                            continue
...
def api_me():
    # 1. Restore session from persistent token (header / cookie / param) if session expired
    if "uid" not in session:
        auth_header = request.headers.get("Authorization", "")
        token_candidate = auth_header[7:].strip() if auth_header.startswith("Bearer ") else ""
        if not token_candidate:
            token_candidate = request.headers.get("X-Domya-Token") or request.cookies.get("domya_token") or request.args.get("token") or request.args.get("t")
        if token_candidate:
            u_info = _verify_session_token(token_candidate)
            if u_info:
                session.permanent = True
    ...
    return jsonify({
        ...
        "employee_id": emp_id,
        "name": emp_name,
        "job": rec.get("job") or emp_info.get("job") or ("Content creator" if is_content_creator() else "Employee"),
...
def api_set_active_client():
    ...
    if cid in ("all", "__all__", "", None):
        session.pop("active_client_id", None)
        session.modified = True
        return jsonify({"ok": True, "active_client_id": None})
...
def api_clients_get():
    ...
    # Enrich every client with AM name and Google Drive URLs for crystal-clear clarity for the Manager/Admin
    ...
    for c in pool:
        ...
        cid = str(c.get("id") or "")
        mfid = client_month_folder_id(cid) or client_drive_folder_id(cid)
        c["drive_folder_url"] = f"https://drive.google.com/drive/folders/{mfid}" if mfid else ""
        c["drive_folder_id"] = mfid or ""

        c_tasks = [t for t in all_tasks if str(t.get("client_id") or "") == cid]
        c["tasks_count"] = len(c_tasks)
        c_plan_task = next((t for t in c_tasks if (t.get("plan_drive_link") or t.get("drive_plan_url"))), None)
        if c_plan_task:
            c["plan_drive_url"] = c_plan_task.get("plan_drive_link") or c_plan_task.get("drive_plan_url")
            c["plan_name"] = c_plan_task.get("plan_name")
        else:
            c["plan_drive_url"] = ""

        c["plan_share_url"] = f"/share/plan/{cid}?t={_client_share_token(cid)}"
...
@app.route("/api/clients/drive-url", methods=["GET"])
@auth_guard
def api_client_drive_url():
    """Returns direct Google Drive links for a specific client (folder & plan file)."""
    cid = (request.args.get("client_id") or request.args.get("cid") or "").strip()
    if not cid:
        return jsonify({"ok": False, "error": "client_id مطلوب"}), 400
    mfid = client_month_folder_id(cid) or client_drive_folder_id(cid)
    folder_url = f"https://drive.google.com/drive/folders/{mfid}" if mfid else ""
    tasks = get_client_tasks(cid)
    plan_task = next((t for t in tasks if (t.get("plan_drive_link") or t.get("drive_plan_url"))), None)
    plan_url = (plan_task.get("plan_drive_link") or plan_task.get("drive_plan_url")) if plan_task else ""
    token = _client_share_token(cid)
    share_url = f"/share/plan/{cid}?t={token}"
    return jsonify({
        "ok": True,
        "client_id": cid,
        "folder_id": mfid or "",
...
def api_clients_add():
    ...
    raw_amid = str(data.get("am_employee_id") or data.get("am_id") or "").strip()
    raw_amname = str(data.get("am_name") or "").strip()
    init_amid = "AM-2072-9827"
    init_amname = "محمود خالد"
    if raw_amid in ("EMP-5887-5256", "AM-5887-5256") or "آيه" in raw_amname or "ايه" in raw_amname:
        init_amid = "EMP-5887-5256"
        init_amname = "آيه أحمد مجاهد"
...
def find_client_in_store(cid, data=None):
    """Robustly find a client in AGENCY_CLIENTS_STORE supporting:
    - Raw ID
    - URL-decoded ID (urllib.parse.unquote)
    - URL-encoded ID (urllib.parse.quote)
    - Latin1 to UTF-8 decoded ID (WSGI PEP 3333 header/path decoding)
    - ID or client_id from request JSON payload
    - Client exact name or company
    - Normalized Arabic name key
    - Automatic Supabase sync fallback if not found
    """
    import urllib.parse
    data = data or {}
    raw_cid = str(cid or "").strip()
    unquoted_cid = urllib.parse.unquote(raw_cid).strip() if raw_cid else ""
    quoted_cid = urllib.parse.quote(raw_cid).strip() if raw_cid else ""
    latin_cid = ""
    try:
        if raw_cid:
            latin_cid = raw_cid.encode("latin1").decode("utf-8").strip()
    except Exception:
        pass

    data_id = str(data.get("id") or data.get("client_id") or "").strip()
    unquoted_data_id = urllib.parse.unquote(data_id).strip() if data_id else ""
    data_name = str(data.get("name") or data.get("client_name") or "").strip()

    def _search():
        target_ids = {raw_cid, unquoted_cid, quoted_cid, latin_cid, data_id, unquoted_data_id}
        target_ids = {x for x in target_ids if x}

        # 1. Exact ID match
        for c in AGENCY_CLIENTS_STORE:
            if str(c.get("id") or "") in target_ids:
                return c

        # 2. Exact Name / Company match
        target_names = {raw_cid, unquoted_cid, latin_cid, data_name}
        target_names = {x for x in target_names if x}
        for c in AGENCY_CLIENTS_STORE:
            c_name = str(c.get("name") or "").strip()
            c_comp = str(c.get("company") or "").strip()
            if c_name in target_names or c_comp in target_names:
                return c

        # 3. Normalized Arabic Name match
        norm_keys = {_normalize_client_name_key(x) for x in target_names if x}
        norm_keys = {k for k in norm_keys if k}
        if norm_keys:
            for c in AGENCY_CLIENTS_STORE:
                c_norm = _normalize_client_name_key(c.get("name") or "")
                if c_norm and c_norm in norm_keys:
                    return c
                c_comp_norm = _normalize_client_name_key(c.get("company") or "")
                if c_comp_norm and c_comp_norm in norm_keys:
                    return c

        # 4. Canonical Default fallback
        for cc in CANONICAL_DEFAULT_CLIENTS:
            cc_id = str(cc.get("id") or "")
            cc_name = str(cc.get("name") or "")
            if cc_id in target_ids or cc_name in target_names or (_normalize_client_name_key(cc_name) in norm_keys if norm_keys else False):
                existing = next((c for c in AGENCY_CLIENTS_STORE if str(c.get("id")) == cc_id), None)
                if existing:
                    return existing
                new_c = dict(cc)
                AGENCY_CLIENTS_STORE.append(new_c)
                push_setting("meta_ai_clients", AGENCY_CLIENTS_STORE)
                return new_c
        return None
...
@app.route("/api/clients/<cid>", methods=["PUT", "POST"])
@require_manager
def api_clients_update(cid):
    data = request.get_json() or {}
    client = find_client_in_store(cid, data)
    if not client:
    ...
    cid_actual = str(client.get("id") or cid)

    am_changed = False
    new_am_id = ""
    new_am_name = ""
    if "am_employee_id" in data or "am_id" in data or "am_name" in data:
        raw_amid = str(data.get("am_employee_id") or data.get("am_id") or "").strip()
        raw_amname = str(data.get("am_name") or "").strip()
        if raw_amid in ("EMP-5887-5256", "AM-5887-5256") or "آيه" in raw_amname or "ايه" in raw_amname:
            new_am_id = "EMP-5887-5256"
            new_am_name = "آيه أحمد مجاهد"
        elif raw_amid in ("AM-2072-9827", "EMP-2072-9827") or "محمود" in raw_amname:
    ...
    if am_changed and new_am_id:
        try:
            all_tasks = _all_tasks_db()
            c_name_norm = _normalize_client_name_key(client.get("name") or "")
            tasks_to_reassign = [
                t for t in all_tasks
                if str(t.get("client_id") or "") == cid_actual
                or (c_name_norm and _normalize_client_name_key(t.get("client_name") or "") == c_name_norm)
            ]
            if tasks_to_reassign:
                for t in tasks_to_reassign:
                    t["am_id"] = new_am_id
                    t["am_name"] = new_am_name
                    logs = t.setdefault("activity_log", [])
                    logs.append({
                        "action": "reassigned_am",
                        "actor_name": current_user_rec().get("name") or "المدير",
                        "actor_type": current_role(),
                        "timestamp": _cairo_now().isoformat(),
                        "note": f"تحديث مسؤول الحسابات للعميل إلى: {new_am_name}"
                    })
                save_client_tasks(tasks_to_reassign, cid_actual)
        except Exception as _e:
            print(f"[api_clients_update cascade err] {_e}")

    return jsonify({"ok": True, "client": client, "am_changed": am_changed})
...
def api_clients_delete(cid):
    ...
    # Match client by ID or Name using robust finder
    client = find_client_in_store(cid, data)
    if not client:
...
def _send_due_task_reminders(force=False):
    """
    Automated Daily Deadline & Reminders Engine:
    1. Sends Telegram alert to assigned Employee 1 day before delivery deadline (تنبيه قبلها بيوم).
    2. Sends Telegram alert to assigned Employee on the delivery day (مستحقة اليوم).
    3. Sends Telegram alert to Account Manager (AM) on the delivery day detailing all tasks due for receipt (موعد استلام التاسكات اليوم).
    """
    now_cairo = _cairo_now()
    today_dt = now_cairo.date()
    ...
    if not force:
        ...
        if now_cairo.hour < 9:
            return 0 # wait until 9 AM Cairo
            
    cache["reminders_sent_date"] = today
    push_setting("meta_ai_reminders_date", today)
    
    active_statuses = {"Assigned", "In Progress", "Pending AM Approval", "Awaiting AM Review"}
    all_tasks = _all_tasks_db()
    
    emp_due_tomorrow = {}  # {eid: [tasks]}
    emp_due_today = {}     # {eid: [tasks]}
    am_due_today = {}      # {am_id_or_name: [tasks]}
    
    for t in all_tasks:
        if not isinstance(t, dict):
            continue
        st = t.get("status") or "Assigned"
        if st in ("Completed", "Approved / Scheduled", "Cancelled"):
            continue
        ...
        am_name = str(t.get("am_name") or "").strip()
        
        # 1. Employee: 1-Day Advance Warning (قبل التسليم بيوم)
        if due == tomorrow and st in ("Assigned", "In Progress"):
            if eid:
                emp_due_tomorrow.setdefault(eid, []).append(t)
            if sec_eid and sec_eid != eid:
                emp_due_tomorrow.setdefault(sec_eid, []).append(t)
            
        # 2. Employee: Due Today or Overdue (مستحقة اليوم أو متأخرة)
        if due <= today and st in ("Assigned", "In Progress"):
            if eid:
                emp_due_today.setdefault(eid, []).append(t)
            if sec_eid and sec_eid != eid:
                emp_due_today.setdefault(sec_eid, []).append(t)
            
        # 3. Account Manager: Due Today or Overdue for Receipt (استلام التاسك النهاردة)
        if due <= today and (am_id or am_name):
            am_key = am_id or am_name
            am_due_today.setdefault(am_key, []).append(t)
            
    sent_count = 0
    portal_url = globals().get("PORTAL_URL", "https://metaaimoderator.vercel.app")
    tasks_url = f"{portal_url}/#tasks"
    myportal_url = f"{portal_url}/#myportal"
    
    # ---- Send to Employees (1-Day Advance Warning) ----
    for eid, tasks in emp_due_tomorrow.items():
        chat = _emp_chat(eid)
        if not chat:
            continue
        emp_name = (_sheet_emp(eid) or {}).get("name", "") or tasks[0].get("assignee_name") or "يا بطل"
        lines = [
            f"⏰ <b>تذكير مسبق: موعد تسليم المهام غداً!</b>",
            f"صباح الخير {emp_name} 👋",
            f"عندك <b>{len(tasks)} مهمة</b> موعد تسليمها غداً ({tomorrow}):\n"
        ]
        for t in tasks[:10]:
            p_num = t.get("post_number_in_plan") or t.get("post_number") or ""
            seq_tag = f"#{p_num} " if p_num else ""
            c_name = t.get("client_name") or _client_name(t.get("client_id")) or ""
            client_tag = f" — [{c_name}]" if c_name else ""
            lines.append(f"📌 {seq_tag}<b>{t.get('title','مهمة')}</b>{client_tag}")
        lines.append(f"\n🔗 <b>افتح بوابتك لتسليم العمل:</b>\n{myportal_url}")
        if send_telegram_bot_notification(chat, "\n".join(lines)):
            sent_count += 1

    # ---- Send to Employees (Due Today Alert) ----
    for eid, tasks in emp_due_today.items():
        chat = _emp_chat(eid)
        if not chat:
            continue
        emp_name = (_sheet_emp(eid) or {}).get("name", "") or tasks[0].get("assignee_name") or "يا بطل"
        lines = [
            f"🚨 <b>تنبيه عاجل: اليوم موعد تسليم المهام!</b>",
            f"صباح الخير {emp_name} 👋",
            f"عندك <b>{len(tasks)} مهمة</b> مستحقة للتسليم اليوم ({today}):\n"
        ]
        for t in tasks[:10]:
            due_d = str(t.get("delivery_deadline") or "")[:10]
            overdue_tag = " [⚠️ متأخرة]" if (due_d and due_d < today) else ""
            p_num = t.get("post_number_in_plan") or t.get("post_number") or ""
            seq_tag = f"#{p_num} " if p_num else ""
            c_name = t.get("client_name") or _client_name(t.get("client_id")) or ""
            client_tag = f" — [{c_name}]" if c_name else ""
            lines.append(f"• {seq_tag}<b>{t.get('title','مهمة')}</b>{client_tag}{overdue_tag}")
        lines.append(f"\n📤 <b>سلّم شغلك الآن عبر الرابط:</b>\n{myportal_url}")
        if send_telegram_bot_notification(chat, "\n".join(lines)):
            sent_count += 1

    # ---- Send to Account Managers (AMs) (Receipt of Tasks Today) ----
    for am_key, tasks in am_due_today.items():
        chat = _emp_chat(am_key)
        if not chat:
            continue
            
        am_name = tasks[0].get("am_name") or (_sheet_emp(am_key) or {}).get("name", "") or "مدير الحساب"
        lines = [
            f"📋 <b>تنبيه الأكونت مانيجر (AM) — مواعيد استلام المهام اليوم:</b>",
            f"صباح الخير {am_name} 👋",
            f"اليوم هو موعد استلام <b>{len(tasks)} مهمة</b> من فريق العمل ({today}):\n"
        ]
        for t in tasks[:12]:
            p_num = t.get("post_number_in_plan") or t.get("post_number") or ""
            seq_tag = f"#{p_num} " if p_num else ""
            c_name = t.get("client_name") or _client_name(t.get("client_id")) or "عميل"
            assignee = t.get("assignee_name") or "غير مسندة"
            st_val = str(t.get("status") or "")
            st_text = "تم التسليم / قيد المراجعة 📤" if (st_val in ("Submitted / In Review", "Awaiting AM Review", "Submitted") or "review" in st_val.lower()) else ("جاري العمل ⏱️" if st_val == "In Progress" else ("مكتملة ومعتمدة ✅" if st_val == "Completed" else "بانتظار البدء 📌"))
            lines.append(f"• {seq_tag}<b>{t.get('title','مهمة')}</b>\n  🏢 العميل: {c_name} | 👤 الموظف: {assignee}\n  📊 الحالة: {st_text}")
        lines.append(f"\n🔍 <b>متابعة واعتماد المهام عبر اللوحة:</b>\n{tasks_url}")
        if send_telegram_bot_notification(chat, "\n".join(lines)):
            sent_count += 1
...
@app.route("/api/tasks/reminders/trigger", methods=["POST", "GET"])
@auth_guard
def api_tasks_reminders_trigger():
    """Manually trigger deadline reminders to AMs and Employees."""
    sync_from_supabase()
    sent = _send_due_task_reminders(force=True)
    return jsonify({
        "success": True,
        "message": f"تم فحص المواعيد وإرسال {sent} تنبيه تليجرام لمديري الحسابات والموظفين بنجاح 🔔",
        "reminders_sent": sent
    })
...
def _sanitize_task_record(d):
    ...
    cr_name = str(d.get("creator_name") or d.get("content_creator_name") or "").strip()
    if cr_id and (not cr_name or cr_name == cr_id):
        res_id, res_name = _resolve_creator_employee(cr_id)
        if res_name:
            d["creator_name"] = res_name
            d["content_creator_name"] = res_name
    elif cr_name and not cr_id:
        res_id, res_name = _resolve_creator_employee(cr_name)
        if res_id:
            d["creator_id"] = res_id
            d["content_creator_id"] = res_id
            if res_name:
                d["creator_name"] = res_name
                d["content_creator_name"] = res_name
...
def _append_task_log(t, action, actor_name=None, actor_id=None, **kwargs):
    """Appends an event to the task's activity log and recalculates KPI metrics."""
    actor_type = kwargs.get("actor_type")
    target_emp_id = kwargs.get("target_emp_id")
    target_emp_name = kwargs.get("target_emp_name")
    ...
    # Check if there is an active modification request or return to employee
    ...
    if mod_at:
        try:
            ...
            # If the task has been submitted after or around modification request
            if not dt_sub or dt_sub >= dt_mod:
                effective_start = mod_at
                kpis["is_modification"] = True
                kpis["modification_requested_at"] = mod_at
                # Use explicit modification_deadline or calculate from mod date if old delivery_deadline expired
                mod_dl = t.get("modification_deadline")
                if mod_dl:
                    effective_deadline = mod_dl
                else:
                    mod_cairo_d = (dt_mod + timedelta(hours=_tz_offset())).date()
                    orig_dl_d = None
                    if deadline:
                        try:
                            ...
                            pass
                    # If old deadline is missing or already passed before modification date, set +1 day
                    if not orig_dl_d or orig_dl_d < mod_cairo_d:
                        effective_deadline = (dt_mod + timedelta(hours=_tz_offset(), days=1)).strftime("%Y-%m-%d")
                    else:
...
def _normalize_client_name_key(s):
    ...
    s = re.sub(r'^(?:خطة\s+محتوى|خطة|عميل|شركة|عيادة|مركز|دكتورة|دكتور|د[\.\/]|dr\.?)\s*', '', s).strip()
    s = re.sub(r'[أإآا]', 'ا', s)
    s = re.sub(r'ة', 'ه', s)
    s = re.sub(r'ى', 'ي', s)
    s = re.sub(r'[\s_\-]+', '', s)
...
def _ensure_client_record(client_input, am_id=None):
    """Guarantees that client_input (name, ID, or alias) resolves to its canonical client record
    without fuzzy substring collision or data cross-contamination."""
    global AGENCY_CLIENTS_STORE
    
    # 0. Handle empty or generic client input
    if not client_input:
        cid = current_client_id()
    ...
    if client_str in ("client_default", "عميل عام", "None", "null"):
        cid = current_client_id()
        if cid and cid not in ("client_default", "None", "null"):
            c = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == cid), None)
            if c:
                return cid, c
        return "client_default", {"id": "client_default", "name": "عميل عام"}

    client_str_low = client_str.lower()
    norm_input = _normalize_client_name_key(client_str)

    target = None

    # 1. Search existing clients by exact ID
    for c in AGENCY_CLIENTS_STORE:
        if not isinstance(c, dict):
            continue
        c_id = str(c.get("id") or "").strip()
        if c_id and c_id.lower() == client_str_low:
            target = c
            break

    # 2. Search existing clients by exact Name or Company
    if not target:
        for c in AGENCY_CLIENTS_STORE:
            if not isinstance(c, dict):
                continue
            ...
            if (c_name and c_name == client_str_low) or (c_comp and c_comp == client_str_low):
                target = c
                break

    # 3. Search via Canonical Aliases (bidirectional Arabic + English)
    if not target:
        for canonical_id, aliases in CANONICAL_CLIENT_ALIASES.items():
            matched = False
            for al in aliases:
                al_low = al.lower()
                al_norm = _normalize_client_name_key(al)
                if client_str_low == al_low or (norm_input and norm_input == al_norm):
                    matched = True
                    break
                if len(al_norm) >= 4 and al_norm == norm_input:
                    matched = True
                    break
            if matched:
                target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == canonical_id), None)
                if not target:
                    target = next((dict(cc) for cc in CANONICAL_DEFAULT_CLIENTS if cc.get("id") == canonical_id), None)
                    if target:
                        AGENCY_CLIENTS_STORE.append(target)
                break

    # 4. Strict Distinctive Brand Token Disambiguation
    if not target:
        # Check distinctive keywords — each brand has unique, mutually exclusive anchors
        if any(k in norm_input or k in client_str_low for k in ["fahmy", "فهمي"]):
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "cli_dr_ahmed_fahmy_1788683119"), None)
        elif any(k in norm_input or k in client_str_low for k in ["hamdy", "حمدي"]):
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "cli_dr_ahmed_1788270119"), None)
        elif any(k in norm_input or k in client_str_low for k in ["شاهنده", "شاهندة", "شاهندا", "shahinda", "shahenda"]):
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "cli_dr_shahenda_1788685119"), None)
        elif any(k in norm_input or k in client_str_low for k in ["حياة", "hayat", "اسنان", "أسنان", "فينير", "فينيرز", "dental"]):
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "cli_hayat_dental_center_1788685057"), None)
        elif any(k in norm_input or k in client_str_low for k in ["حافظ", "hafez", "هبهحافظ", "هبةحافظ"]):
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "cli_هبه_حافظ_1788431922"), None)
        elif any(k in norm_input or k in client_str_low for k in ["شيماء", "shimaa", "shymaa", "عاطف", "atef"]):
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "cli_dr_shimaa_atef_1788298157"), None)
        elif any(k in norm_input or k in client_str_low for k in ["هدير", "hadeer"]):
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "cli_dr_hadeer_1788684282"), None)
        elif any(k in norm_input or k in client_str_low for k in ["رعاية", "reaya", "rayah"]) and "عام" not in norm_input:
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "cli_معامل_رعاية_1788336726"), None)
        elif any(k in norm_input or k in client_str_low for k in ["انفينيتي", "infinity"]):
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "cli_انفينيتي_1788270119"), None)
        elif norm_input in ("sk", "عيادهsk", "اسكي", "اسكى") or client_str_low in ("sk", "sk clinic"):
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "cli_sk_1788270118"), None)
        elif any(k in norm_input or k in client_str_low for k in ["domya", "دمية", "دميه"]):
            target = next((cl for cl in AGENCY_CLIENTS_STORE if cl.get("id") == "client_100821894800009"), None)

    # 5. Target resolved
    if target:
        target_id = str(target.get("id"))
        # Ensure canonical client always keeps its authentic official name
        canonical_def = next((cc for cc in CANONICAL_DEFAULT_CLIENTS if cc.get("id") == target_id), None)
        if canonical_def:
            target["name"] = canonical_def["name"]
            target["company"] = canonical_def["company"]
        if not target.get("am_employee_id"):
            if am_id:
                ...
                canonical_def = next((cc for cc in CANONICAL_DEFAULT_CLIENTS if cc.get("id") == target_id), None)
                if canonical_def and canonical_def.get("am_employee_id"):
                    target["am_employee_id"] = canonical_def["am_employee_id"]
                    target["am_name"] = canonical_def.get("am_name") or "محمود خالد"
    ...
    # 6. Guard against creating dummy clients from generic strings
    if not norm_input or norm_input in ("عام", "عميل", "عميلعام", "none", "null", "undefined", "default"):
        return "client_default", {"id": "client_default", "name": "عميل عام"}

    # 7. Create new client record for genuine new clients
    ...
    # SAFETY: Guard against polluting global client list during tests or with phantom test names
    is_testing = bool(os.environ.get("PYTEST_CURRENT_TEST") or any("pytest" in str(arg).lower() for arg in sys.argv))
    try:
        from flask import current_app
        if current_app and current_app.config.get("TESTING"):
            is_testing = True
    ...
    if not is_testing and not any(kw in client_str.lower() for kw in ("نور", "النور", "test", "تجرب", "creator_test")):
        AGENCY_CLIENTS_STORE.append(new_client)
        push_setting("meta_ai_clients", AGENCY_CLIENTS_STORE)
        try:
            import threading
...
def client_drive_folder_id(cid):
    ...
    if not cid:
        return None
    if str(cid) in CANONICAL_CLIENT_PARENT_FOLDERS:
        return CANONICAL_CLIENT_PARENT_FOLDERS[str(cid)]
    try:
        resolved_cid, _ = _ensure_client_record(cid)
        if resolved_cid and str(resolved_cid) in CANONICAL_CLIENT_PARENT_FOLDERS:
            return CANONICAL_CLIENT_PARENT_FOLDERS[str(resolved_cid)]
    except Exception:
        pass
...
def client_month_folder_id(cid, month=None):
    ...
    # If the requested month is the canonical month (2026-09), return the verified monthly folder directly
    if month == "2026-09":
        if str(cid) in CANONICAL_CLIENT_FOLDERS:
            return CANONICAL_CLIENT_FOLDERS[str(cid)]
        try:
            resolved_cid, _ = _ensure_client_record(cid)
            if resolved_cid and str(resolved_cid) in CANONICAL_CLIENT_FOLDERS:
                return CANONICAL_CLIENT_FOLDERS[str(resolved_cid)]
        except Exception:
            pass
...
def _ensure_task_deliverable_in_client_drive(t, cid, drive_link, filename=None):
    """Automatically organize and route any task deliverable (uploaded or pasted)
    directly into the client's Google Drive folder without mixing, and create employee shortcuts."""
    if not drive_link:
        return
    cid = cid or (t.get("client_id") if isinstance(t, dict) else None) or (t.get("clientId") if isinstance(t, dict) else None)
    if not cid:
        return
    fid = drive_file_id(drive_link)
    if not fid or len(fid) < 15:
        return
    ...
    def _worker():
        try:
            ...
            # 1. Ensure file is readable by anyone with the link (zero 403 errors)
            _drive_set_anyone_reader(token, fid)

            # 2. Target client canonical monthly folder
            client_fid = client_month_folder_id(cid) or client_drive_folder_id(cid)
            if not client_fid:
                return

            # 3. Check current parents of the file
            url_f = f"https://www.googleapis.com/drive/v3/files/{fid}?fields=id,name,parents&supportsAllDrives=true"
            req_f = urllib.request.Request(url_f, headers={"Authorization": f"Bearer {token}"})
            finfo = json.loads(urllib.request.urlopen(req_f, timeout=10).read().decode("utf-8"))
            parents = finfo.get("parents") or []
            cur_name = filename or finfo.get("name") or "ملف تسليم"

            # 4. Link directly into Client's Folder if not already a parent
            if client_fid not in parents:
                url_patch = f"https://www.googleapis.com/drive/v3/files/{fid}?addParents={client_fid}&fields=id,parents&supportsAllDrives=true"
                req_p = urllib.request.Request(url_patch, data=b"{}", headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"}, method="PATCH")
                try:
                    urllib.request.urlopen(req_p, timeout=10)
                except Exception:
                    # Fallback: create shortcut in client's folder
                    sc_meta = {
                        "name": cur_name,
                        "mimeType": "application/vnd.google-apps.shortcut",
                        "parents": [client_fid],
            ...
            # 5. Create shortcut in employee's plan folder for easy reference
            eid = str(t.get("assigned_employee_id") or "") if isinstance(t, dict) else ""
            emp_name = str(t.get("assignee_name") or "") if isinstance(t, dict) else ""
            plan_name = str(t.get("plan_name") or t.get("file_name") or _client_name(cid) or "خطة").strip() if isinstance(t, dict) else "خطة"
            sec_eid = str(t.get("secondary_employee_id") or "") if isinstance(t, dict) else ""
            sec_name = str(t.get("secondary_assignee_name") or "") if isinstance(t, dict) else ""

            for emp_id, e_name in [(eid, emp_name), (sec_eid, sec_name)]:
                if emp_id and emp_id.lower() not in ("none", "unassigned", "null", ""):
                    ef = employee_plan_folder_id(emp_id, plan_name, name=e_name)
                    if ef and ef != client_fid and ef not in parents:
                        try:
                            chk_q = urllib.parse.quote(f"'{ef}' in parents and trashed = false")
                            chk_req = urllib.request.Request(
                                f"https://www.googleapis.com/drive/v3/files?q={chk_q}&fields=files(id,shortcutDetails)&pageSize=50&supportsAllDrives=true",
                                headers={"Authorization": f"Bearer {token}"}
                            )
                            chk_res = json.loads(urllib.request.urlopen(chk_req, timeout=10).read().decode("utf-8"))
                            already_in_ef = any(
                                f_item.get("id") == fid or (f_item.get("shortcutDetails") or {}).get("targetId") == fid
                                for f_item in chk_res.get("files", [])
                            )
                            if not already_in_ef:
                                sc_emp = {
                                    "name": cur_name,
                                    "mimeType": "application/vnd.google-apps.shortcut",
                                    "parents": [ef],
                                ...
                                sc_r = urllib.request.Request(
                                    "https://www.googleapis.com/drive/v3/files?fields=id&supportsAllDrives=true",
                                    data=json.dumps(sc_emp).encode("utf-8"),
                                    headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
                                )
                                urllib.request.urlopen(sc_r, timeout=10)
                        except Exception:
                            pass
        except Exception as _e:
            print(f"[_ensure_task_deliverable_in_client_drive] {_e}")
...
def _stop_task_timer_if_running(t):
    """Safely stop running timer and accumulate elapsed time when task is submitted or completed."""
    if not isinstance(t, dict):
        return
    timer_state = t.get("timer_state")
    if timer_state and timer_state.get("is_running"):
        if timer_state.get("last_start"):
            try:
                start_dt = datetime.fromisoformat(timer_state["last_start"])
                elapsed = (datetime.now(timezone.utc) - start_dt).total_seconds()
                timer_state["elapsed_seconds"] = (timer_state.get("elapsed_seconds") or 0) + int(elapsed)
...
def api_drive_asset():
    ...
    save_client_tasks(tasks, _cid)
    try:
        _ensure_task_deliverable_in_client_drive(target_task, _cid, drive_link)
    except Exception:
        pass
...
@app.route("/api/tasks/plan/add-task", methods=["POST", "PUT"])
@auth_guard
def api_tasks():
    _cid = current_client_id()
    if request.method == "POST":
        ...
        publish_date = (data.get("publish_date") or scheduled_start or "").strip()
        delivery_deadline = (data.get("delivery_deadline") or scheduled_start or "").strip()
        if not delivery_deadline and not publish_date:
            delivery_deadline = datetime.now().strftime("%Y-%m-%d")
            publish_date = delivery_deadline
        elif not delivery_deadline and publish_date:
            delivery_deadline = publish_date
        elif not publish_date and delivery_deadline:
            publish_date = delivery_deadline
        drive_link = (data.get("drive_link") or "").strip()
        ...
        sec_emp_id = (data.get("secondary_employee_id") or data.get("co_employee_id") or "").strip()
        sec_emp_name = (data.get("secondary_assignee_name") or data.get("co_assignee_name") or "").strip()
        if sec_emp_id and not sec_emp_name:
            sec_info = _sheet_emp(sec_emp_id)
            sec_emp_name = sec_info.get("name") or sec_emp_id
            
        initial_status = "Assigned" if emp_id else (data.get("status") or "Pending AM Approval")

        cr_id = (data.get("creator_id") or data.get("content_creator_id") or "").strip()
        cr_name = (data.get("creator_name") or data.get("content_creator_name") or "").strip()
        if cr_id:
            resolved_cr_id, resolved_cr_name = _resolve_creator_employee(cr_id)
            cr_id = resolved_cr_id or cr_id
            cr_name = resolved_cr_name or cr_name
        elif cr_name:
            resolved_cr_id, resolved_cr_name = _resolve_creator_employee(cr_name)
            cr_id = resolved_cr_id or ""
            cr_name = resolved_cr_name or cr_name

        # Resolve plan_name, file_name, and post_type
        plan_name = str(data.get("plan_name") or data.get("file_name") or "").strip()
        file_name = str(data.get("file_name") or plan_name).strip()
        post_type = str(data.get("post_type") or data.get("content_type") or "post").lower()
        caption = str(data.get("caption") or description).strip()
        visual_idea = str(data.get("visual_idea") or data.get("design_brief") or "").strip()

        # Compute next sequential post_number in the plan if plan_name is provided
        post_num = data.get("post_number")
        if (post_num is None or post_num == "" or post_num == 0) and plan_name:
            client_tasks = get_client_tasks(_cid) or []
            max_num = 0
            for ct in client_tasks:
                if (ct.get("plan_name") or ct.get("file_name") or "").strip() == plan_name:
                    pn = ct.get("post_number") or _resolve_post_number(ct, default_index=0)
                    if pn and pn > max_num:
                        max_num = pn
            post_num = max_num + 1
        ...
        new_task = {
            ...
            "plan_name": plan_name or f"خطة {_client_name(_cid)}",
            "file_name": file_name or plan_name or f"خطة {_client_name(_cid)}",
            "title": title,
            "tagline": data.get("tagline") or title,
            "tag_line": data.get("tagline") or title,
            "description": description or caption,
            "caption": caption,
            "visual_idea": visual_idea,
            "visual_content": visual_idea,
            "design_brief": visual_idea,
            "content_type": "Carousel" if post_type == "carousel" else ("Video" if post_type in ("reel", "video") else "Single Image"),
            "post_type": post_type,
            "status": initial_status,
            "scheduled_start_date": scheduled_start,
            "publish_date": publish_date,
            "publish_time": data.get("publish_time") or "10:00",
            ...
            "content_creator_id": cr_id,
            "content_creator_name": cr_name,
            "assigned_employee_id": emp_id,
            "assignee_name": emp_name,
            "secondary_employee_id": sec_emp_id or None,
            "secondary_assignee_name": sec_emp_name or None,
            "content_data": data.get("content_data") or {"caption": caption, "visual_idea": visual_idea, "post_type": post_type, "reference_images": data.get("reference_images") or data.get("media_urls") or [], "reference_links": data.get("reference_links") or ([data.get("reference_link")] if data.get("reference_link") else [])},
            "graphic_data": data.get("graphic_data") or {"idea": visual_idea, "reference_images": data.get("reference_images") or data.get("media_urls") or [], "reference_links": data.get("reference_links") or ([data.get("reference_link")] if data.get("reference_link") else [])},
            "video_data": data.get("video_data") or {"script": caption, "idea": visual_idea, "reference_links": data.get("reference_links") or ([data.get("reference_link")] if data.get("reference_link") else [])},
            "media_urls": data.get("media_urls") or data.get("reference_images") or [],
            "reference_images": data.get("reference_images") or data.get("media_urls") or [],
            "reference_link": data.get("reference_link") or "",
            "reference_links": data.get("reference_links") or ([data.get("reference_link")] if data.get("reference_link") else []),
        ...
        if sec_emp_id:
            new_task["co_assigned_at"] = datetime.now(timezone.utc).isoformat()
            
        _append_task_log(new_task, "created",
                         actor_name=current_user_rec().get("name") or current_username(),
                         actor_id=am_id,
                         actor_type=current_role(),
                         note=f"إضافة المهمة مباشرة للخطة: {plan_name}" if plan_name else "إنشاء المهمة يدوياً")
        if emp_id:
        ...
        if drive_link:
            try:
                _ensure_task_deliverable_in_client_drive(new_task, _cid, drive_link)
            except Exception:
                pass
        invalidate_tasks_cache()
        
        tg_sent = False
        if emp_id and data.get("notify_telegram") is not False:
            try:
                tg_sent = bool(send_task_to_employee(new_task, _cid))
            except Exception as _e:
                print(f"[create task notify err] {_e}")
                    
        return jsonify({"ok": True, "success": True, "task": new_task, "telegram_sent": tg_sent})
    ...
    for t in tasks:
        ...
        # Self-healing creator resolution
        cr_id = str(t.get("creator_id") or t.get("content_creator_id") or "").strip()
        if cr_id:
            if not t.get("creator_name") or t.get("creator_name") == cr_id:
                t["creator_name"] = emp_map.get(cr_id) or _resolve_creator_employee(cr_id)[1] or cr_id
            t["content_creator_id"] = cr_id
            t["content_creator_name"] = t.get("creator_name")
        elif t.get("creator_name") and not t.get("content_creator_name"):
            t["content_creator_name"] = t["creator_name"]

        if t.get("client_id"):
            canonical_cname = _client_name(t["client_id"])
            if canonical_cname and canonical_cname not in ("عميل عام", "client_default"):
                t["client_name"] = canonical_cname
            elif not t.get("client_name"):
                t["client_name"] = canonical_cname
...
def _parse_plan_with_ai(plan_text, retry_split=False):
    """Uses Groq LLaMA-3.3 or OpenRouter to intelligently parse marketing plans of ANY structure with STRICT 100% VERBATIM FIDELITY.
    ...
    if retry_split:
        prompt = (
            "أنت نظام استخراج خطط محتوى وتسويق صارم وعالي الأمانة (High-Fidelity Verbatim Content Plan Parser).\n"
            "المستخدم رفع خطة محتوى والمطلوب استخراج كل بوست/مهمة كعنصر منفصل مع أمانة نصية مطلقة 100% دون أي تعديل أو اختصار.\n\n"
            "⚠️ قواعد صارمة وحاسمة وممنوع كسرها نهائياً (STRICT NON-NEGOTIABLE RULES):\n"
            "1. الأمانة النصية الحرفية المطلقة (100% VERBATIM EXTRACTION):\n"
            "   - ممنوع منعاً باتاً تلخيص، أو تقصير، أو اختصار، أو حذف، أو إعادة صياغة، أو 'تحسين' أي كلمة أو جملة أو إيموجي أو رقم أو رابط كتبه صانع المحتوى.\n"
            "   - حقل caption: يجب أن يحتوي على نص المحتوى / الكابشن / الإسكربت الكامل كما هو بالحرف وبدقة 100%، بجميع فقراته وسلايداته وسياقه التام.\n"
            "   - حقل visual_idea: يجب أن يحفظ كافة التوجيهات الإبداعية والمشاهد والأفكار البصرية المذكورة في النص كما هي بالكامل.\n"
            "   - ممنوع استخدام علامات الحذف (...) أو استبدال النص بعبارات مثل [باقي النص] أو اختصار النقاط الطبية/التسويقية.\n"
            "2. فصل الهوك والعنوان بدون مساس بنص الكابشن:\n"
            "   - حقل title: الهوك أو العنوان للبوست (أول سطر أو الفكرة الرئيسية).\n"
            "   - حقل tagline: الهوك أو التاج لاين المحدد إن وُجد.\n"
            "   - إذا كنت غير متأكد من أين ينتهي الهوك، احتفظ بالنص كاملاً داخل caption ولا تحذف أي حرف.\n"
            "3. كل بوست منفصل = عنصر منفصل:\n"
            "   - كل منشور أو ريلز أو كاروسيل أو فيديو = كائن منفصل داخل مصفوفة posts.\n\n"
            "أرجع JSON فقط بالشكل التالي دون أي نص إضافي:\n"
            ...
            "لا تكتب أي شيء خارج كود الـ JSON."
        )
    else:
        prompt = (
            "You are an expert, high-fidelity Social Media & Marketing Content Plan Parser.\n"
            "Your sole mission is to extract ALL individual posts from the given marketing plan with 100% STRICT VERBATIM FIDELITY.\n\n"
            "NON-NEGOTIABLE RULES FOR COMPLETE DATA PRESERVATION:\n"
            "1. ZERO DATA LOSS / 100% STRICT VERBATIM COPY:\n"
            "   - NEVER summarize, NEVER shorten, NEVER paraphrase, NEVER translate, NEVER 'improve', and NEVER omit ANY word, sentence, line, bullet point, emoji, punctuation, phone number, or CTA from the author's copy.\n"
            "   - The 'caption' field MUST BE an exact, complete, word-for-word copy of the entire post body / copy / script as written by the copywriter.\n"
            "   - The 'visual_idea' field MUST preserve all creative directions, slide descriptions, visual concepts, and references without cutting them.\n"
            "   - NEVER replace text with ellipses (...) or placeholders like '[continued]'. Output every single word in full.\n"
            "2. HOOK & TITLE EXTRACTION:\n"
            "   - 'title': Extract the clean hook or topic headline (max 120 chars).\n"
            "   - 'tagline': The specific hook/headline or opening hook.\n"
            "   - In 'caption', KEEP THE COMPLETE POST BODY intact without amputating beginning sentences.\n"
            "3. SEPARATION OF POSTS:\n"
            "   - Each separate post, reel, story, video, carousel, or motion graphic = ONE separate object in 'posts'.\n"
            "   - If the plan is a TABLE, each data row is a separate post.\n\n"
            "Return ONLY a JSON object with key 'posts' containing an array of objects:\n"
            "- title: (string) Clean post headline or subject\n"
            "- tagline: (string) The specific Hook / Tagline / Headline\n"
            "- caption: (string) The COMPLETE 100% verbatim copy/body/script\n"
            "- visual_idea: (string) Complete design / visual / video scene description or reference\n"
            "- post_type: (string) 'post' | 'reel' | 'carousel' | 'story' | 'motion'\n"
            "- publish_date: (string) 'YYYY-MM-DD' if specified, else ''\n"
            "- publish_time: (string) 'HH:MM' (default '10:00')\n"
            "- delivery_deadline: (string) 'YYYY-MM-DD' if specified, else ''\n"
            "- media_urls: (array of strings) Any URLs or links in this post\n\n"
        ...
        try:
            ...
            body = {
                ...
                "messages": [
                    ...
                    {"role": "user", "content": plan_text[:120000]}
                ],
    ...
    # Try Groq first (fastest, with 10s timeout)
    if GROQ_API_KEY:
        posts = _try_api("https://api.groq.com/openai/v1/chat/completions", GROQ_API_KEY, "llama-3.3-70b-versatile", 10 if not retry_split else 8)
        if posts:
            return posts

    # Fallback to OpenRouter (fast, with 10s timeout)
    if OPENROUTER_API_KEY:
        posts = _try_api("https://openrouter.ai/api/v1/chat/completions", OPENROUTER_API_KEY, "meta-llama/llama-3.3-70b-instruct:free", 10 if not retry_split else 8)
        if posts:
            return posts
...
def _universal_extract_plan_posts(plan_text):
    ...
    for idx, p in enumerate(final_posts, 1):
        ...
        # Clean prefixes from title and tagline
        title = re.sub(r'^(?:كونتنت|محتوى|كابشن|ديزاين)\s*[:：]\s*', '', title, flags=re.I).strip()
        tag = re.sub(r'^(?:كونتنت|محتوى|كابشن|ديزاين)\s*[:：]\s*', '', tag, flags=re.I).strip()
...
def _extract_structured_docx_plan(file_bytes, filename="", parent_id=None, uploader_client_name=""):
    ...
    if filename_lower.endswith(".docx") or file_bytes[:2] == b"PK":
        try:
            ...
            # Detect Client, Month & Content Creator from paragraphs
            for p in doc.paragraphs:
                ...
                m_cr = re.search(r'(?:Content\s*Creator|Copywriter|Writer|الكونتنت\s*كريتور|كاتب\s*المحتوى|صانع\s*المحتوى|كتابة|إعداد|معد\s*الخطة|المعد|بواسطة|By|Author)\s*[:=\-]\s*(.+)', t, re.IGNORECASE)
                if m_cr and not doc_creator:
                    doc_creator = m_cr.group(1).strip()
...
def _generate_plan_docx(client_name, plan_name, posts, am_name=None, creator_name=None):
    ...
    for idx, p in enumerate(posts, 1):
        ...
        pillar_map = {
            "education": "💡 تثقيفي وتوعوي (Education)",
            "authority": "👑 سلطة وخبرة (Authority)",
            "social_proof": "🌟 آراء وثقة (Social Proof)",
            "conversion": "🎯 عرض وبيعي (Conversion)",
            "viral": "🔥 تريند وتفاعل (Viral & Engagement)"
        }
        raw_pillar = p.get("content_pillar") or p.get("pillar") or ""
        pillar_label = pillar_map.get(raw_pillar, raw_pillar)

        add_row("🏷️ التاج لاين / الهوك:", tagline, RGBColor(180, 83, 9), is_bold=True)
        if pillar_label:
            add_row("🎯 الركيزة التسويقية:", pillar_label, RGBColor(21, 128, 61), is_bold=True)
        if visual:
...
# KNOWN_EMPLOYEE_ROSTER and KNOWN_CREATOR_ALIASES are defined early in api/index.py for fast bootstrap.



def _resolve_creator_employee(creator_input):
    """Matches a creator input (employee ID, name, or alias) to an Agency employee record.
    Guarantees robust O(1) resolution without network dependency, returning (employee_id, employee_name)."""
    if not creator_input or not isinstance(creator_input, str):
        return ("", "")
    
    inp = creator_input.strip()
    if not inp:
        return ("", "")

    # Clean input from role prefixes
    clean_inp = re.sub(
        r'^(?:content\s*creator|copywriter|writer|الكونتنت\s*كريتور|كاتب\s*المحتوى|صانع\s*المحتوى|كاتب|المعد|بواسطة|إعداد|by)\s*[:=\-]*\s*',
        '', inp, flags=re.I
    ).strip()
    ...
    try:
        ...
        for e in cached_emps:
            if not isinstance(e, dict):
                continue
            e_id = str(e.get("employee_id") or "").strip()
    ...
    if is_emp_id_format:
        ...
        # Check if single employee record is known via _sheet_emp
        try:
            e_rec = _sheet_emp(canonical_id)
            if e_rec and e_rec.get("name"):
                return (canonical_id, e_rec["name"])
        except Exception:
            pass
    ...
    if name_clean and name_clean.lower() not in ("none", "null", "auto", "unassigned", "undefined", "اختيار", "تعيين"):
        ...
        return (new_eid, name_clean)

    return ("", clean_inp or inp)


def _save_and_distribute_plan_to_drive(client_id, client_name, plan_name, posts, **kwargs):
    ...
    4. Assigned Employees' Google Drive Folders (مجلدات كل الموظفين المسند لهم مهام في الخطة)
    And sends confirmation to both on Telegram with the Google Drive link."""
    am_id = kwargs.get("am_id")
    am_name = kwargs.get("am_name")
    creator_id = kwargs.get("creator_id")
    creator_name = kwargs.get("creator_name")
    try:
        clean_pname = (plan_name or f"خطة {client_name}").strip()
        
        # 0. Resolve Content Creator from arguments, posts, or session
        c_eid = creator_id or ""
        c_name = creator_name or ""
        if not c_eid or not c_name:
            for p in (posts or []):
                if isinstance(p, dict):
                    cand_id = p.get("creator_id") or p.get("content_creator_id")
                    cand_name = p.get("creator_name") or p.get("content_creator_name")
                    if cand_id:
                        c_eid = c_eid or str(cand_id).strip()
                        c_name = c_name or str(cand_name).strip()
                        break
        if not c_eid:
            try:
                c_eid = _my_employee_id() or session.get("uid") or ""
            except Exception:
                c_eid = ""
        if c_eid:
            res_id, res_name = _resolve_creator_employee(c_eid)
            c_eid = res_id or c_eid
            c_name = res_name or c_name
        if not c_name:
            try:
                c_name = current_user_rec().get("name") or current_username() or "فريق المحتوى"
            except Exception:
                c_name = "فريق المحتوى"

        docx_bytes = _generate_plan_docx(client_name, clean_pname, posts, am_name=am_name, creator_name=c_name)
        ...
        am_fid = employee_plan_folder_id(amid, f"{client_name} - {clean_pname}", name=amnm) or employee_drive_folder_id(amid, name=amnm)
        
        creator_fid = None
        if c_eid and c_eid != amid:
            creator_fid = employee_plan_folder_id(c_eid, f"{client_name} - {clean_pname}", name=c_name) or employee_drive_folder_id(c_eid, name=c_name)
            
        # Collect all assigned employees' plan folders (and any other creators)
        emp_fids = []
        seen_eids = {amid, c_eid} if c_eid else {amid}
        for p in (posts or []):
            if isinstance(p, dict):
                # Assigned Employee
                p_eid = str(p.get("assigned_employee_id") or "").strip()
                p_ename = str(p.get("assignee_name") or "").strip()
                if p_eid and p_eid.lower() not in ("none", "unassigned", "null") and p_eid not in seen_eids:
                    seen_eids.add(p_eid)
                    efid = employee_plan_folder_id(p_eid, f"{client_name} - {clean_pname}", name=p_ename) or employee_drive_folder_id(p_eid, name=p_ename)
                    if efid:
                        emp_fids.append(efid)
                
                # Secondary Assignee / Co-Assignee
                p_seid = str(p.get("secondary_employee_id") or "").strip()
                p_sname = str(p.get("secondary_assignee_name") or "").strip()
                if p_seid and p_seid.lower() not in ("none", "unassigned", "null") and p_seid not in seen_eids:
                    seen_eids.add(p_seid)
                    sefid = employee_plan_folder_id(p_seid, f"{client_name} - {clean_pname}", name=p_sname) or employee_drive_folder_id(p_seid, name=p_sname)
                    if sefid:
                        emp_fids.append(sefid)

                # Creator on post if different
                p_cid = str(p.get("creator_id") or p.get("content_creator_id") or "").strip()
                p_cname = str(p.get("creator_name") or p.get("content_creator_name") or "").strip()
                if p_cid and p_cid.lower() not in ("none", "unassigned", "null") and p_cid not in seen_eids:
                    seen_eids.add(p_cid)
                    cfid = employee_plan_folder_id(p_cid, f"{client_name} - {clean_pname}", name=p_cname) or employee_drive_folder_id(p_cid, name=p_cname)
                    if cfid:
                        emp_fids.append(cfid)

        target_fids = [fid for fid in [client_fid, am_fid, creator_fid] + emp_fids if fid]
        if not target_fids:
...
def _ensure_task_plan_in_employee_drive(task, cid):
    """When a task is assigned or reassigned to an employee, ensure the employee (and content creator)
    has the plan folder and plan DOCX shortcut in their Drive."""
    try:
        if not isinstance(task, dict):
            return
        eid = str(task.get("assigned_employee_id") or "").strip()
        ename = str(task.get("assignee_name") or "").strip()
        c_eid = str(task.get("creator_id") or task.get("content_creator_id") or "").strip()
        c_name = str(task.get("creator_name") or task.get("content_creator_name") or "").strip()

        cname = task.get("client_name") or _client_name(cid) or "عميل"
        pname = str(task.get("plan_name") or task.get("file_name") or f"خطة {cname}").strip()
        folder_display_name = f"{cname} - {pname}"
        safe_fname = f"{cname} — {pname}.docx"

        token = get_google_oauth_access_token()
        if not token:
            return

        drive_plan_url = task.get("plan_drive_link") or task.get("drive_plan_url")
        target_doc_id = drive_file_id(drive_plan_url) if drive_plan_url else None
        if not target_doc_id:
            all_c_tasks = get_client_tasks(cid)
            for other_t in all_c_tasks:
                if (other_t.get("plan_name") or other_t.get("file_name")) == pname:
                    u = other_t.get("plan_drive_link") or other_t.get("drive_plan_url")
                    if u:
                        target_doc_id = drive_file_id(u)
                        if target_doc_id:
                            break

        # 1. Sync Assigned Employee folder
        if eid and eid.lower() not in ("none", "unassigned", "null"):
            emp_plan_fid = employee_plan_folder_id(eid, folder_display_name, name=ename) or employee_drive_folder_id(eid, name=ename)
            if emp_plan_fid and target_doc_id:
                existing_doc = _drive_find_folder_by_name(emp_plan_fid, safe_fname)
                if not existing_doc:
                    sc_meta = {
                        "name": safe_fname,
                        "mimeType": "application/vnd.google-apps.shortcut",
                        "parents": [emp_plan_fid],
                        "shortcutDetails": {"targetId": target_doc_id}
                    }
                    sc_req = urllib.request.Request(
                        "https://www.googleapis.com/drive/v3/files?fields=id",
                        data=json.dumps(sc_meta).encode("utf-8"),
        ...
        # 2. Sync Content Creator folder
        if c_eid and c_eid.lower() not in ("none", "unassigned", "null") and c_eid != eid:
            res_cid, res_cname = _resolve_creator_employee(c_eid)
            c_eid = res_cid or c_eid
            c_name = res_cname or c_name
            c_plan_fid = employee_plan_folder_id(c_eid, folder_display_name, name=c_name) or employee_drive_folder_id(c_eid, name=c_name)
            if c_plan_fid and target_doc_id:
                existing_c_doc = _drive_find_folder_by_name(c_plan_fid, safe_fname)
                if not existing_c_doc:
                    sc_meta_c = {
                        "name": safe_fname,
                        "mimeType": "application/vnd.google-apps.shortcut",
                        "parents": [c_plan_fid],
                    ...
                    sc_req_c = urllib.request.Request(
                        "https://www.googleapis.com/drive/v3/files?fields=id",
                        data=json.dumps(sc_meta_c).encode("utf-8"),
                        headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
                    )
                    urllib.request.urlopen(sc_req_c, timeout=10)

        # 3. Sync Secondary Assignee folder (Co-Assignee)
        sec_eid = str(task.get("secondary_employee_id") or "").strip()
        sec_name = str(task.get("secondary_assignee_name") or "").strip()
        if sec_eid and sec_eid.lower() not in ("none", "unassigned", "null") and sec_eid != eid and sec_eid != c_eid:
            sec_plan_fid = employee_plan_folder_id(sec_eid, folder_display_name, name=sec_name) or employee_drive_folder_id(sec_eid, name=sec_name)
            if sec_plan_fid and target_doc_id:
                existing_sec_doc = _drive_find_folder_by_name(sec_plan_fid, safe_fname)
                if not existing_sec_doc:
                    sc_meta_sec = {
                        "name": safe_fname,
                        "mimeType": "application/vnd.google-apps.shortcut",
                        "parents": [sec_plan_fid],
                        "shortcutDetails": {"targetId": target_doc_id}
                    }
                    sc_req_sec = urllib.request.Request(
                        "https://www.googleapis.com/drive/v3/files?fields=id",
                        data=json.dumps(sc_meta_sec).encode("utf-8"),
                        headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
                    )
                    urllib.request.urlopen(sc_req_sec, timeout=10)

        if not target_doc_id:
            plan_tasks = [t for t in get_client_tasks(cid) if (t.get("plan_name") or t.get("file_name")) == pname] or [task]
            _save_and_distribute_plan_to_drive(cid, cname, pname, plan_tasks, creator_id=c_eid, creator_name=c_name)
    except Exception as _e:
        print(f"[_ensure_task_plan_in_employee_drive error] {_e}")


def _infer_sequential_post_date(plan_name="", idx=0, total_posts=1):
    """Generates an evenly spaced, realistic sequential deadline across the plan's target month."""
    try:
        now = datetime.now()
        target_year = now.year
        target_month = now.month
        p_str = str(plan_name or "").lower()
        ...
        tot = max(1, total_posts)
        day = min(28, max(2, round(((idx + 1) / tot) * 26 + 1)))
...
def api_tasks_ingest_plan():
    try:
        ...
        am_name = am_emp.get("name") or (current_user_rec().get("name") if str(am_id) == str(_my_employee_id()) else "")
        ...
        if up and up.filename:
            ...
            if docx_res and docx_res.get("posts"):
                ...
                doc_creator_detected = docx_res.get("creator") or ""
        ...
        if plan_text and not doc_creator_detected:
            m_cr = re.search(r'(?:Content\s*Creator|Copywriter|Writer|الكونتنت\s*كريتور|كاتب\s*المحتوى|صانع\s*المحتوى|كتابة|إعداد|معد\s*الخطة|المعد|بواسطة|By|Author)\s*[:=\-]\s*(.+)', plan_text, re.IGNORECASE)
            if m_cr:
                doc_creator_detected = m_cr.group(1).splitlines()[0].strip()
        ...
        # Check if raw_file_name or doc_client_detected contains a specific brand
        detected_brand_input = ""
        if doc_client_detected:
            detected_brand_input = doc_client_detected
        elif raw_file_name:
            for can_id, aliases in CANONICAL_CLIENT_ALIASES.items():
                if can_id == "client_100821894800009":
                    continue  # do not override non-Domya files with Domya
                for al in aliases:
                    if len(al) >= 4 and al.lower() in raw_file_name.lower():
                        detected_brand_input = can_id
                        break
                if detected_brand_input:
                    break

        client_input = ""
        cid_param_clean = str(client_id_param or "").strip()
        cname_param_clean = str(client_name_param or "").strip()

        # If frontend passed a specific client, strictly respect user selection!
        if cid_param_clean in ("cli_شاهندا_1788860117", "cli_dr_shahenda_1788685119"):
            client_input = "cli_dr_shahenda_1788685119"
        elif cid_param_clean and cid_param_clean not in ("all", "__all__", "client_default"):
            client_input = cid_param_clean
        elif cname_param_clean and cname_param_clean not in ("عميل عام", "العميل", "None", "null", "all", "__all__"):
            client_input = cname_param_clean
        elif detected_brand_input:
            client_input = detected_brand_input
        elif current_client_id() and current_client_id() not in ("client_default", "all", "__all__"):
        ...
        # If am_id was not explicitly passed, inherit the client's assigned Account Manager!
        if not am_id and target_client:
            am_id = str(target_client.get("am_employee_id") or target_client.get("am_id") or "").strip()
        if not am_id:
            am_id = "AM-2072-9827"
        if not am_name:
            if am_id in ("EMP-5887-5256", "AM-5887-5256"):
                am_name = "آيه أحمد مجاهد"
            elif am_id in ("AM-2072-9827", "EMP-2072-9827"):
                am_name = "محمود خالد"
            else:
                am_name = (target_client.get("am_name") if target_client else "") or "محمود خالد"
        ...
        elif c_name_disp and sub_title and (sub_title.lower().startswith(c_name_disp.lower()) or sub_title.lower().startswith(f"خطة {c_name_disp.lower()}") or c_name_disp.lower() in sub_title.lower()):
        ...
        has_explicit_no_append = bool(
            (request.form.get("append") in ("false", "0") if request.form else False) or
            (isinstance(req_json, dict) and (req_json.get("append") is False or req_json.get("allow_append") is False))
        )
        has_explicit_append = bool(
            (request.form.get("append") in ("true", "1") if request.form else False) or
            (isinstance(req_json, dict) and (req_json.get("append") is True or req_json.get("allow_append") is True))
        )
        allow_append = bool(has_explicit_append and not has_explicit_no_append)
        ...
        # If appending to existing plan, sequence smoothly; otherwise, keep distinct plans isolated!
        if dup_plan_name and dup_count > 0:
            if allow_append:
                ...
                # Multiple distinct plans in the same month: give this plan its own clean name (e.g. خطة ... (2))
                cand = clean_file_title
                idx_n = 2
                while _norm_plan_str(cand) in existing_plan_index:
                    cand = f"{clean_file_title} ({idx_n})"
                    idx_n += 1
                clean_file_title = cand
                base_post_offset = 0
        else:
            base_post_offset = 0

        # Resolve Content Creator: Param > Detected in Doc > Session
        creator_param = (request.form.get("content_creator_id") or request.form.get("creator_id") if request.form else None) or \
                        (req_json.get("content_creator_id") or req_json.get("creator_id") if isinstance(req_json, dict) else "")
        creator_name_param = (request.form.get("content_creator_name") or request.form.get("creator_name") if request.form else None) or \
                             (req_json.get("content_creator_name") or req_json.get("creator_name") if isinstance(req_json, dict) else "")
        creator_input = str(creator_param or creator_name_param or "").strip()
        has_explicit_creator = bool(creator_input and creator_input.lower() not in ("auto", "none", "null"))
        if not has_explicit_creator:
            creator_input = doc_creator_detected or ""
        if not creator_input:
            creator_input = _my_employee_id() or session.get("uid") or ""

        resolved_creator_id, resolved_creator_name = _resolve_creator_employee(creator_input)
        if creator_name_param and (not resolved_creator_name or resolved_creator_name == resolved_creator_id):
            resolved_creator_name = str(creator_name_param).strip()
        if not has_explicit_creator:
            if not resolved_creator_id and _my_employee_id():
                resolved_creator_id = _my_employee_id()
            if not resolved_creator_name:
                resolved_creator_name = current_user_rec().get("name") or current_username() or "فريق المحتوى"

        # Resolve Global Assignee if specified at plan level
        global_assigned_eid = (request.form.get("assigned_employee_id") or request.form.get("assignee_id") if request.form else None) or \
                              (req_json.get("assigned_employee_id") or req_json.get("assignee_id") if isinstance(req_json, dict) else "")
        global_assigned_name = (request.form.get("assignee_name") or request.form.get("assigned_employee_name") if request.form else None) or \
                               (req_json.get("assignee_name") or req_json.get("assigned_employee_name") if isinstance(req_json, dict) else "")
        global_assigned_eid = str(global_assigned_eid or "").strip()
        global_assigned_name = str(global_assigned_name or "").strip()
        if global_assigned_eid:
            _reid, _rname = _resolve_creator_employee(global_assigned_eid)
            global_assigned_eid = _reid or global_assigned_eid
            if _rname and (not global_assigned_name or global_assigned_name == global_assigned_eid):
                global_assigned_name = _rname
        elif global_assigned_name:
            _reid, _rname = _resolve_creator_employee(global_assigned_name)
            global_assigned_eid = _reid
            global_assigned_name = _rname or global_assigned_name

        if global_assigned_name:
            global_assigned_name = re.sub(r'^[^\w\u0600-\u06FF]+', '', global_assigned_name)
            global_assigned_name = re.sub(r'\s*[\(—\-].*$', '', global_assigned_name).strip()

        # Resolve Global Secondary Assignee / Co-Assignee if specified at plan level
        global_sec_eid = (request.form.get("secondary_employee_id") or request.form.get("co_assignee_id") if request.form else None) or \
                         (req_json.get("secondary_employee_id") or req_json.get("co_assignee_id") if isinstance(req_json, dict) else "")
        global_sec_name = (request.form.get("secondary_assignee_name") or request.form.get("co_assignee_name") if request.form else None) or \
                          (req_json.get("secondary_assignee_name") or req_json.get("co_assignee_name") if isinstance(req_json, dict) else "")
        global_sec_eid = str(global_sec_eid or "").strip()
        global_sec_name = str(global_sec_name or "").strip()
        if global_sec_eid:
            _reid, _rname = _resolve_creator_employee(global_sec_eid)
            global_sec_eid = _reid or global_sec_eid
            if _rname and (not global_sec_name or global_sec_name == global_sec_eid):
                global_sec_name = _rname
        elif global_sec_name:
            _reid, _rname = _resolve_creator_employee(global_sec_name)
            global_sec_eid = _reid
            global_sec_name = _rname or global_sec_name

        if global_sec_name:
            global_sec_name = re.sub(r'^[^\w\u0600-\u06FF]+', '', global_sec_name)
            global_sec_name = re.sub(r'\s*[\(—\-].*$', '', global_sec_name).strip()
        ...
        for p_idx, p in enumerate(extracted_posts, 1):
            ...
            assigned_eid = (p.get("assigned_employee_id") or global_assigned_eid or "").strip()
            assigned_name = (p.get("assignee_name") or global_assigned_name or "").strip()
            if assigned_eid:
                _reid, _rname = _resolve_creator_employee(assigned_eid)
                assigned_eid = _reid or assigned_eid
                if _rname and (not assigned_name or assigned_name == assigned_eid):
                    assigned_name = _rname
            elif assigned_name:
                _reid, _rname = _resolve_creator_employee(assigned_name)
                assigned_eid = _reid
                assigned_name = _rname or assigned_name

            if assigned_name:
                assigned_name = re.sub(r'^[^\w\u0600-\u06FF]+', '', assigned_name)
                assigned_name = re.sub(r'\s*[\(—\-].*$', '', assigned_name).strip()

            sec_eid = (p.get("secondary_employee_id") or global_sec_eid or "").strip()
            sec_name = (p.get("secondary_assignee_name") or global_sec_name or "").strip()
            if sec_eid:
                _rseid, _rsname = _resolve_creator_employee(sec_eid)
                sec_eid = _rseid or sec_eid
                if _rsname and (not sec_name or sec_name == sec_eid):
                    sec_name = _rsname
            elif sec_name:
                _rseid, _rsname = _resolve_creator_employee(sec_name)
                sec_eid = _rseid
                sec_name = _rsname or sec_name

            if sec_name:
                sec_name = re.sub(r'^[^\w\u0600-\u06FF]+', '', sec_name)
                sec_name = re.sub(r'\s*[\(—\-].*$', '', sec_name).strip()

            # Support per-post creator if explicitly specified on postObj, fallback to plan creator
            p_cid = str(p.get("creator_id") or p.get("content_creator_id") or "").strip()
            p_cname = str(p.get("creator_name") or p.get("content_creator_name") or "").strip()
            p_cand = p_cid or p_cname
            if p_cand:
                post_res_id, post_res_name = _resolve_creator_employee(p_cand)
                final_creator_id = post_res_id or p_cid or resolved_creator_id
                final_creator_name = post_res_name or p_cname or resolved_creator_name
            else:
                final_creator_id = resolved_creator_id
                final_creator_name = resolved_creator_name

            if final_creator_id:
                _rc_id, _rc_name = _resolve_creator_employee(final_creator_id)
                if _rc_name and (not final_creator_name or final_creator_name == final_creator_id):
                    final_creator_name = _rc_name
            if final_creator_name:
                final_creator_name = re.sub(r'^[^\w\u0600-\u06FF]+', '', final_creator_name)
                final_creator_name = re.sub(r'\s*[\(—\-].*$', '', final_creator_name).strip()
            ...
            new_task = {
                ...
                "creator_name": final_creator_name,
                "content_creator_id": final_creator_id,
                "content_creator_name": final_creator_name,
                "assigned_employee_id": assigned_eid,
                "assignee_name": assigned_name,
                "secondary_employee_id": sec_eid,
                "secondary_assignee_name": sec_name,
            ...
            if sec_eid:
                _append_task_log(new_task, "co_assigned",
                                 actor_name=new_task.get("am_name"),
                                 actor_id=new_task.get("am_id"),
                                 actor_type="account_manager",
                                 target_emp_id=sec_eid,
                                 target_emp_name=sec_name,
                                 note=f"إسناد كشريك عمل للمهمة: {sec_name}")
        ...
        # Automatically generate DOCX and save to Drive (Client folder, AM folder, Creator folder, Assignee folders)
        ...
        if tg:
            ...
            am_notified = bool(send_telegram_bot_notification(tg,
                ...
                f"✍️ من: {resolved_creator_name or 'فريق المحتوى'}\n"
                f"{drive_msg}\n"
                f"راجع وحدّد المواعيد والإسناد من البوابة 🚀"))

        # Notify Creator on Telegram if creator != AM and has telegram ID
        if resolved_creator_id and resolved_creator_id != am_id:
            cr_emp = _sheet_emp(resolved_creator_id)
...
def api_plan_create():
    ...
    creator_param = (data.get("content_creator_id") or data.get("creator_id") or "").strip()
    creator_name_param = (data.get("content_creator_name") or data.get("creator_name") or "").strip()
    creator_input = creator_param or creator_name_param
    has_explicit_creator = bool(creator_input and creator_input.lower() not in ("auto", "none", "null"))
    if not has_explicit_creator:
        m_cr = re.search(r'(?:Content\s*Creator|Copywriter|Writer|الكونتنت\s*كريتور|كاتب\s*المحتوى|صانع\s*المحتوى|كتابة|إعداد|معد\s*الخطة|المعد|بواسطة|By|Author)\s*[:=\-]\s*(.+)', plan_text, re.IGNORECASE)
        if m_cr:
            creator_input = m_cr.group(1).splitlines()[0].strip()
        else:
            creator_input = _my_employee_id() or session.get("uid") or ""

    resolved_creator_id, resolved_creator_name = _resolve_creator_employee(creator_input)
    if creator_name_param and (not resolved_creator_name or resolved_creator_name == resolved_creator_id):
        resolved_creator_name = creator_name_param
    if not has_explicit_creator:
        if not resolved_creator_id and _my_employee_id():
            resolved_creator_id = _my_employee_id()
        if not resolved_creator_name:
    ...
    for p_idx, p in enumerate(extracted_posts, 1):
        ...
        new_t = {
            ...
            "post_number": post_num,
            "client_id": client_id,
            "client_name": target_client.get("name") or _client_name(client_id),
            ...
            "creator_id": resolved_creator_id,
            "creator_name": resolved_creator_name,
            "content_creator_id": resolved_creator_id,
            "content_creator_name": resolved_creator_name,
...
def api_share_feedback():
    ...
    if not cid:
        return jsonify({"error": "معرف العميل غير موجود", "ok": False}), 400
    ...
    if not is_valid:
        return jsonify({"error": "رمز التحقق غير صالح", "ok": False}), 403
    ...
    if t:
        ...
        if decision == "approved":
            t["client_decision"] = "approved"
            t["client_approved_at"] = datetime.now(timezone.utc).isoformat()
        elif decision == "changes_requested":
            t["client_decision"] = "changes_requested"
            t["client_feedback_note"] = note
            t["client_changes_requested_at"] = datetime.now(timezone.utc).isoformat()
    ...
    if am_tg:
        ...
        status_label = "معتمد وممتاز ✅" if decision == "approved" else "طلب تعديل / ملاحظات ✏️"
    ...
    return jsonify({"ok": True, "success": True, "message": "تم إرسال ردك لفريق العمل بنجاح!"})
...
def share_plan_view(client_id):
    ...
    plan_drive_url = next((t.get("plan_drive_link") or t.get("drive_plan_url") for t in tasks if (t.get("plan_drive_link") or t.get("drive_plan_url"))), "")
    client_fid = client_month_folder_id(cid) or client_drive_folder_id(cid)
    folder_url = f"https://drive.google.com/drive/folders/{client_fid}" if client_fid else ""

    drive_btns_html = ""
    if plan_drive_url:
        drive_btns_html += f'''
        <a href="{_hx(plan_drive_url)}" target="_blank" class="inline-flex items-center gap-1.5 bg-white text-slate-900 hover:bg-purple-50 text-xs font-bold px-3.5 py-2 rounded-xl shadow transition">
            <span>📄 فتح ملف الخطة الأصلي (Word / Google Drive) ↗</span>
        </a>'''
    if folder_url:
        drive_btns_html += f'''
        <a href="{_hx(folder_url)}" target="_blank" class="inline-flex items-center gap-1.5 bg-white/20 hover:bg-white/30 text-white text-xs font-bold px-3 py-2 rounded-xl border border-white/20 transition">
            <span>📁 مجلد محتوى العميل على Drive ↗</span>
        </a>'''
    ...
    for idx, t in enumerate(tasks, 1):
        ...
        client_dec = str(t.get("client_decision") or "")
        if client_dec == "approved":
            react_badge = f'<span id="react-status-{_hx(tid)}" class="text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-lg">✅ معتمد وممتاز</span>'
        elif client_dec == "changes_requested":
            react_badge = f'<span id="react-status-{_hx(tid)}" class="text-[11px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-lg">✏️ تم إرسال طلب التعديل</span>'
        else:
            react_badge = f'<span id="react-status-{_hx(tid)}" class="text-[11px] font-bold hidden"></span>'
        ...
        <div class="bg-white border border-slate-200/90 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4 hover:shadow-md transition">
            ...
            <div class="caption-box space-y-1.5">
                <div class="flex items-center justify-between text-xs font-bold text-slate-700">
                    <span>📝 نص المنشور والكابشن (Caption):</span>
                    <button type="button" onclick="copyCardCaption(this)" class="text-[11px] bg-slate-100 hover:bg-blue-50 hover:text-blue-700 text-slate-600 px-2.5 py-1 rounded-lg border border-slate-200 transition flex items-center gap-1 cursor-pointer">
                        <span>📋 نسخ الكابشن</span>
                    </button>
                </div>
                <div class="caption-content p-3.5 bg-slate-50 border border-slate-200/70 rounded-2xl text-xs sm:text-sm text-slate-800 leading-relaxed whitespace-pre-line select-text font-sans">
            ...
            <div class="pt-3 border-t border-slate-100 flex items-center justify-between flex-wrap gap-2">
                <div id="action-btns-{_hx(tid)}" class="flex items-center gap-2">
...
<body class="bg-slate-100 text-slate-900 antialiased min-h-screen py-6 px-3 sm:px-6">
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <header class="bg-gradient-to-r from-slate-900 via-indigo-950 to-purple-950 text-white rounded-3xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
            <div class="relative z-10 space-y-4">
                ...
                {f'<div class="flex items-center gap-2 flex-wrap pt-1">{drive_btns_html}</div>' if drive_btns_html else ''}
            </div>
        </header>
    ...
    <script>
        ...
        function copyCardCaption(btn) {{
            var box = btn ? btn.closest('.caption-box') : null;
            var el = box ? box.querySelector('.caption-content') : null;
            var text = el ? (el.innerText || el.textContent || '') : '';
            if (!text.trim()) {{
                showToast('لا يوجد كابشن للنسخ');
                return;
            }}
        ...
        async function sendClientReaction(taskId, decision, note) {{
            var btns = document.getElementById('action-btns-' + taskId);
            ...
            try {{
                ...
                if (data.ok) {{
                    ...
                    if (st) {{
                        st.textContent = decision === 'approved' ? '✅ معتمد وممتاز' : '✏️ تم إرسال طلب التعديل';
                        st.className = 'text-[11px] font-bold ' + (decision === 'approved' ? 'text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-lg' : 'text-amber-700 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-lg');
                        st.classList.remove('hidden');
                    }}
                }} else {{
                    showToast('تعذّر تسجيل الرد: ' + (data.error || 'خطأ غير متوقع'));
                    if (btns) {{
                        var buttons = btns.querySelectorAll('button');
                        for (var i = 0; i < buttons.length; i++) {{
...
def api_task_upload_asset(task_id):
    ...
    if not t:
        return jsonify({"error": "المهمة غير موجودة"}), 404
    # the assignee (employee or co-assignee) OR a manager/admin on this client may upload
    my_eid = _my_employee_id()
    if not (can_see_client(cid) or str(t.get("assigned_employee_id") or "") == my_eid or str(t.get("secondary_employee_id") or "") == my_eid):
        return jsonify({"error": "غير مصرح"}), 403
    up = request.files.get("file") if request.files else None
    if not up or not up.filename:
        return jsonify({"error": "لم يتم اختيار ملف"}), 400
    ...
    # Store primarily in Client Monthly Folder, with secondary shortcuts in Employee folders
    client_fid = client_month_folder_id(cid) or client_drive_folder_id(cid)
    ...
    target_pids = [p for p in [client_fid, emp_fid, emp_direct_fid] if p]
    
    sec_eid = str(t.get("secondary_employee_id") or "").strip()
    sec_name = str(t.get("secondary_assignee_name") or "").strip()
    if sec_eid and sec_eid.lower() not in ("none", "unassigned", "null", ""):
        sec_fid = employee_plan_folder_id(sec_eid, plan_name, name=sec_name) or employee_drive_folder_id(sec_eid, name=sec_name)
        sec_direct_fid = employee_direct_deliverables_folder_id(sec_eid, name=sec_name)
        for sf in [sec_fid, sec_direct_fid]:
            if sf and sf not in target_pids:
                target_pids.append(sf)
    ...
    if not ext:
        if mime == "application/pdf" or "pdf" in str(mime).lower():
            ext = ".pdf"
    ...
    if mime.startswith("video") or ext.lower() in [".mp4", ".mov", ".webm", ".avi", ".mkv"]:
        t["media_type"] = "video"
    elif mime == "application/pdf" or ext.lower() == ".pdf":
        t["media_type"] = "pdf"
    else:
        t["media_type"] = "image"
...
def api_task_upload_session(task_id):
    ...
    plan_name = str(t.get("plan_name") or t.get("file_name") or _client_name(cid) or "خطة عامة").strip()
    post_num = t.get("post_number") or _resolve_post_number(t, default_index=1)
    target_folder_id = client_month_folder_id(cid) or client_drive_folder_id(cid) or employee_plan_folder_id(eid, plan_name, name=emp_name)
...
def api_task_upload_complete(task_id):
    """Finalize a direct upload: make the file link-readable, ensure it is in the client's folder,
    create employee shortcuts, and attach it to the task."""
    t, cid = _find_task_any_client(task_id)
    if not t:
        return jsonify({"error": "المهمة غير موجودة"}), 404
    if not (is_admin() or is_manager() or is_content_creator() or can_see_client(cid) or str(t.get("assigned_employee_id") or "") == _my_employee_id() or _my_employee_id()):
    ...
    if token:
        ...
        # Ensure file is linked in Client's Month Folder AND create shortcuts in employee folder(s)
        client_fid = client_month_folder_id(cid) or client_drive_folder_id(cid)
        eid = str(t.get("assigned_employee_id") or _my_employee_id() or "")
        emp_name = t.get("assignee_name") or current_user_rec().get("name") or eid
        plan_name = str(t.get("plan_name") or t.get("file_name") or _client_name(cid) or "خطة عامة").strip()
        emp_fid = employee_plan_folder_id(eid, plan_name, name=emp_name) if eid else None
        emp_direct_fid = employee_direct_deliverables_folder_id(eid, name=emp_name) if eid else None
        sec_eid = str(t.get("secondary_employee_id") or "").strip()
        sec_name = str(t.get("secondary_assignee_name") or "").strip()
        sec_fid = employee_plan_folder_id(sec_eid, plan_name, name=sec_name) if sec_eid else None
        sec_direct_fid = employee_direct_deliverables_folder_id(sec_eid, name=sec_name) if sec_eid else None

        if client_fid:
            try:
                req_add = urllib.request.Request(
                    f"https://www.googleapis.com/drive/v3/files/{fid}?addParents={client_fid}&supportsAllDrives=true&fields=id",
                    data=b"{}",
                    headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"},
                    method="PATCH"
                )
        ...
        for ef in [emp_fid, emp_direct_fid, sec_fid, sec_direct_fid]:
            if ef and ef != client_fid:
                try:
                    sc_name = (request.get_json() or {}).get("filename") or f"{_client_name(cid)} — {t.get('title')}"
                    sc_meta = {
                        "name": sc_name,
                        "mimeType": "application/vnd.google-apps.shortcut",
                        "parents": [ef],
    ...
    if str(mime).startswith("video") or ext in [".mp4", ".mov", ".webm", ".avi", ".mkv"]:
        t["media_type"] = "video"
    elif str(mime) == "application/pdf" or ext == ".pdf":
        t["media_type"] = "pdf"
    else:
        t["media_type"] = "image"
...
def api_tasks_update_content(task_id):
    ...
    if "notes" in data or "review_note" in data or "modification_request" in data:
        ...
        if n_val:
            now_iso = datetime.now(timezone.utc).isoformat()
            target_task["modification_requested_at"] = now_iso
            cairo_now = datetime.now(timezone.utc) + timedelta(hours=_tz_offset())
            orig_dl = str(target_task.get("delivery_deadline") or "")[:10]
            if not orig_dl or orig_dl < cairo_now.strftime("%Y-%m-%d"):
                target_task["modification_deadline"] = (cairo_now + timedelta(days=1)).strftime("%Y-%m-%d")
            else:
                target_task["modification_deadline"] = orig_dl
    ...
    for t in _all_tasks_db():
        ...
        if not is_adm:
            if is_mgr:
                ...
                t_sec_id = str(t.get("secondary_employee_id") or "").strip()
                if my_eid and t_emp_id != my_eid and t_sec_id != my_eid:
                    continue
...
def api_tasks_monthly_report():
    ...
    for t in tasks:
        ...
        elif sec_nm and sec_nm in name_to_eid and name_to_eid[sec_nm] in stats:
            sec_target_key = name_to_eid[sec_nm]
        elif sec_eid:
            sec_target_key = sec_eid
        ...
        for target_key in keys_to_credit:
            ...
            effective_dl = t.get("modification_deadline") or t.get("delivery_deadline")
            if is_on_time is None and effective_dl and t.get("submitted_at"):
                dl = str(effective_dl)[:10]
            ...
            effective_start = t.get("modification_requested_at") or t.get("returned_to_employee_at") or t.get("assigned_at")
            if th is None and effective_start and t.get("submitted_at"):
                try:
                    dt_a = datetime.fromisoformat(str(effective_start).replace("Z", "+00:00"))
...
def api_tasks_send_monthly_report():
    ...
    for r in report_rows:
        ...
        deliv = r.get('submitted', 0)
        comp = r.get('completed', 0)
        rows_html += f"<tr><td style='padding:8px;border:1px solid #e2e8f0;font-weight:bold;'>{_hx(r['employee'])} <span style='font-size:11px;color:#64748b;'>({_hx(r.get('role',''))})</span></td><td style='padding:8px;border:1px solid #e2e8f0;text-align:center;'>{r['assigned']}</td><td style='padding:8px;border:1px solid #e2e8f0;text-align:center;'>{r.get('in_progress', 0)}</td><td style='padding:8px;border:1px solid #e2e8f0;text-align:center;font-weight:bold;color:#2563eb;'>{deliv}</td><td style='padding:8px;border:1px solid #e2e8f0;text-align:center;font-weight:bold;color:#16a34a;'>{comp}</td><td style='padding:8px;border:1px solid #e2e8f0;text-align:center;font-weight:bold;'>{r['completion_rate']}</td><td style='padding:8px;border:1px solid #e2e8f0;text-align:center;font-weight:bold;color:#16a34a;'>{r.get('on_time_rate','-')}</td><td style='padding:8px;border:1px solid #e2e8f0;text-align:center;'>{r.get('avg_turnaround','-')}</td><td style='padding:8px;border:1px solid #e2e8f0;text-align:center;'>{r.get('avg_duration','-')}</td><td style='padding:8px;border:1px solid #e2e8f0;font-size:11px;'>{notes_html}</td></tr>"

    generated_at = datetime.now(timezone.utc).strftime("%d/%m/%Y - %I:%M %p UTC")
    html_content = f"""<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><style>body{{font-family:Arial,sans-serif;background:#f8fafc;padding:20px;color:#1e293b;}}table{{border-collapse:collapse;width:100%;background:#ffffff;border-radius:8px;overflow:hidden;}}th{{background:#2563eb;color:white;padding:10px;text-align:right;}}td{{padding:8px;border:1px solid #e2e8f0;}}</style></head><body><div style="max-width:900px;margin:0 auto;background:#ffffff;padding:25px;border-radius:12px;box-shadow:0 4px 6px rgba(0,0,0,0.05);"><h2 style="color:#2563eb;margin-top:0;text-align:center;">📊 تقرير أداء ومؤشرات الـ KPIs للفريق - {month_str}</h2><p style="font-size:14px;color:#64748b;text-align:center;">تاريخ التقرير: <strong>{month_str}</strong> | وقت التقديم: {generated_at}</p><table><thead><tr><th>الموظف</th><th>المسندة</th><th>جاري العمل</th><th>تم التسليم</th><th>المعتمدة</th><th>معدل الإنجاز</th><th>الالتزام بالموعد</th><th>متوسط مدة الإنجاز</th><th>وقت التايمر</th><th>الملاحظات</th></tr></thead><tbody>{rows_html if rows_html else "<tr><td colspan='10' style='text-align:center;padding:15px;'>لا توجد مهام مسجلة لهذا الشهر بعد</td></tr>"}</tbody></table><p style="font-size:12px;color:#94a3b8;margin-top:25px;text-align:center;">تم إصدار هذا التقرير تلقائياً من منصة Domya Meta AI Task & Operations Engine 🚀</p></div></body></html>"""
...
_HR_DEFAULTS = {
    ...
    "geofence_meters": int(os.environ.get("ATT_GEOFENCE_M", 50)),
...
def api_my_tasks():
    ...
    sync_from_supabase()
    target_eid = (request.args.get("employee_id") or "").strip()
    is_adm = is_admin()
    
    # If admin explicitly requests all employees
    if is_adm and target_eid == "all":
    ...
    # Strictly sort and return personal assigned tasks only (no accidental team dump)
    mine.sort(key=_natural_task_sort_key)
    return jsonify({"tasks": mine, "employee_id": eid or "me", "is_admin": is_adm})
...
def api_tasks_assign(task_id):
    ...
    if "secondary_employee_id" in data:
        sec_id = str(data.get("secondary_employee_id") or "").strip()
        if sec_id and sec_id.lower() not in ("none", "unassigned", "null", "clear", ""):
            sec_info = _sheet_emp(sec_id)
            t["secondary_employee_id"] = sec_id
            t["secondary_assignee_name"] = sec_info.get("name") or sec_id
        else:
            t["secondary_employee_id"] = None
            t["secondary_assignee_name"] = None
...
@app.route("/api/tasks/<task_id>/co-assign", methods=["POST", "PUT"])
@auth_guard
def api_tasks_co_assign(task_id):
    """Assign or remove a co-assignee (secondary employee) for shared task collaboration."""
    t, cid = _find_task_any_client(task_id)
    if not t:
        return jsonify({"error": "المهمة غير موجودة"}), 404
    if not (is_admin() or is_manager() or can_see_client(cid)):
        return jsonify({"error": "غير مصرح"}), 403

    data = request.get_json(silent=True) or {}
    sec_id = str(data.get("secondary_employee_id") or data.get("employee_id") or "").strip()
    
    if not sec_id or sec_id.lower() in ("none", "unassigned", "null", "clear", ""):
        # Clearing co-assignee
        old_sec_name = t.get("secondary_assignee_name")
        t["secondary_employee_id"] = None
        t["secondary_assignee_name"] = None
        _append_task_log(t, "co_assignee_cleared",
                         actor_name=current_user_rec().get("name") or current_username(),
                         actor_type="account_manager",
                         note=f"إلغاء شريك العمل ({old_sec_name or 'الموظف الثاني'}) من المهمة")
        save_one_task(t, cid)
        return jsonify({"ok": True, "task": t, "secondary_employee_id": None, "secondary_assignee_name": None})

    sec_info = _sheet_emp(sec_id)
    sec_name = sec_info.get("name") or sec_id
    tg = str(sec_info.get("telegram_id", "")).replace(".0", "").strip()

    t["secondary_employee_id"] = sec_id
    t["secondary_assignee_name"] = sec_name
    t["co_assigned_at"] = datetime.now(timezone.utc).isoformat()
    
    _append_task_log(t, "co_assigned",
                     actor_name=current_user_rec().get("name") or current_username(),
                     actor_type="account_manager",
                     target_emp_id=sec_id,
                     target_emp_name=sec_name,
                     note=f"إسناد المهمة كشريك عمل إلى: {sec_name}",
                     details={"primary_assignee": t.get("assignee_name"), "delivery_deadline": t.get("delivery_deadline"), "publish_date": t.get("publish_date")})
    save_one_task(t, cid)

    # Sync Google Drive folder shortcuts for the co-assignee
    try:
        threading.Thread(target=_ensure_task_plan_in_employee_drive, args=(t, cid), daemon=True).start()
    except Exception:
        pass

    # Send Telegram notification to the co-assignee
    sent = False
    if tg:
        portal = os.environ.get("PORTAL_URL", "https://metaaimoderator.vercel.app").rstrip("/")
        deadline = t.get("delivery_deadline") or t.get("publish_date") or "غير محدد"
        cname = _client_name(cid)
        p_assignee = t.get("assignee_name") or "زميلك"
        msg = (f"👥 <b>تم إضافتك كشريك عمل في مهمة!</b>\n"
               f"👤 زميلك في المهمة: <b>{p_assignee}</b>\n"
               f"📌 المهمة: <b>{t.get('title','')}</b>\n"
               f"🏢 العميل: {cname}\n"
               f"📅 موعد التسليم: {deadline}\n"
               f"🔗 افتح بوابتي للتعاون والتسليم: {portal}/#myportal")
        sent = bool(send_telegram_bot_notification(tg, msg))

    return jsonify({"ok": True, "task": t, "secondary_employee_id": sec_id, "secondary_assignee_name": sec_name, "telegram_sent": sent})


@app.route("/api/tasks/<task_id>/assign-creator", methods=["POST", "PUT"])
@require_manager
def api_tasks_assign_creator(task_id):
    """AM/admin assigns a specific Content Creator to a task."""
    t, cid = _find_task_any_client(task_id)
    if not t:
        return jsonify({"error": "المهمة غير موجودة"}), 404
    if not can_see_client(cid):
    ...
    data = request.get_json(silent=True) or {}
    creator_id = str(data.get("creator_id") or "").strip()
    ...
    if not creator_id and not creator_name:
        ...
        t["creator_id"] = None
        t["creator_name"] = None
        t["content_creator_name"] = None
        t["writer_name"] = None
        _append_task_log(t, "creator_cleared",
                         actor_name=current_user_rec().get("name") or current_username(),
                         actor_type="account_manager",
                         note="إلغاء تعيين كاتب المحتوى من المهمة")
        save_one_task(t, cid)
        return jsonify({"ok": True, "task": t, "creator_id": None, "creator_name": None})

    if creator_id and not creator_name:
        emp_info = _sheet_emp(creator_id)
        creator_name = emp_info.get("name") or creator_id

    t["creator_id"] = creator_id
    t["creator_name"] = creator_name
    t["content_creator_name"] = creator_name
    t["writer_name"] = creator_name
    _append_task_log(t, "creator_assigned",
                     actor_name=current_user_rec().get("name") or current_username(),
                     actor_type="account_manager",
                     target_emp_id=creator_id,
                     target_emp_name=creator_name,
                     note=f"تعيين كاتب المحتوى بواسطة مدير الحساب: {creator_name}")
    save_one_task(t, cid)
    return jsonify({"ok": True, "task": t, "creator_id": creator_id, "creator_name": creator_name})
...
def api_plans_assign_bulk():
    ...
    sec_emp_id = str(data.get("secondary_employee_id") or data.get("co_assignee_id") or "").strip()
    sec_emp_info = _sheet_emp(sec_emp_id) if sec_emp_id else {}
    sec_emp_name = sec_emp_info.get("name") or sec_emp_id if sec_emp_id else ""
    sec_tg = str(sec_emp_info.get("telegram_id", "")).replace(".0", "").strip() if sec_emp_info else ""
    ...
    for t in target_tasks:
        ...
        if sec_emp_id:
            t["secondary_employee_id"] = sec_emp_id
            t["secondary_assignee_name"] = sec_emp_name
            _append_task_log(t, "co_assigned",
                             actor_name=actor,
                             actor_type="account_manager",
                             target_emp_id=sec_emp_id,
                             target_emp_name=sec_emp_name,
                             note=f"إسناد كشريك عمل للمهمة: {sec_emp_name}",
                             details={"plan_name": plan_name})
        save_one_task(t, cid)
        assigned_count += 1
        if cid:
    ...
    if tg and assigned_count > 0:
        summary_msg = (f"🚀 <b>تم إسناد خطة محتوى كاملة لك!</b>\n\n"
                       ...
                       + (f"👥 شريك العمل: <b>{sec_emp_name}</b>\n" if sec_emp_name else "")
                       + f"\n🔗 افتح بوابتي للاطلاع على كل مهامك وتسليمها:\n{portal}/#myportal")
        try:
    ...
    sec_tg_sent = False
    if sec_tg and assigned_count > 0:
        sec_msg = (f"👥 <b>تم إسنادك كشريك عمل في خطة محتوى كاملة!</b>\n\n"
                   f"👤 شريكك في العمل: <b>{emp_name}</b>\n"
                   f"🏢 العميل: <b>{cname}</b>\n"
                   f"📑 الخطة: <b>{plan_name}</b>\n"
                   f"🔢 عدد المهام المشتركة: <b>{assigned_count} مهام</b>\n\n"
                   f"🔗 افتح بوابتي للتعاون وتسليم المهام:\n{portal}/#myportal")
        try:
            sec_tg_sent = bool(send_telegram_bot_notification(sec_tg, sec_msg))
        except Exception as _e:
            print(f"[bulk assign sec_tg error] {_e}")
    ...
    return jsonify({
        ...
        "secondary_employee_id": sec_emp_id,
...
def _client_name(cid):
    if not cid or cid in ("client_default", "None", "null"):
        return "عميل عام"
    cid_str = str(cid).strip()
    for cc in CANONICAL_DEFAULT_CLIENTS:
        if str(cc.get("id")) == cid_str:
            return cc.get("name")
    for c in AGENCY_CLIENTS_STORE:
        if str(c.get("id")) == cid_str and c.get("name"):
            return c.get("name")
    return cid_str
...
def api_my_task_start(task_id):
    ...
    t["timer_state"] = ts
    actor_name = emp_name or current_username() or t.get("assignee_name")
    actor_id = eid or _my_employee_id() or t.get("assigned_employee_id")
    _append_task_log(t, "started",
                     actor_name=actor_name,
                     actor_id=actor_id,
                     actor_type="employee",
                     note=f"بدء العمل على المهمة وتشغيل التايمر بواسطة {actor_name}")
    save_one_task(t, cid)
    return jsonify({"ok": True, "task": t})
...
def api_my_task_submit(task_id):
    ...
    t["notes"] = notes
    t["submitted_at"] = datetime.now(timezone.utc).isoformat()
    submitter_name = emp_name or current_user_rec().get("name") or current_username() or "الموظف"
    submitter_eid = eid or _my_employee_id() or t.get("assigned_employee_id")
    t["submitted_by"] = submitter_name
    t["submitted_by_id"] = submitter_eid
    ...
    if drive_link:
        ...
        try:
            _ensure_task_deliverable_in_client_drive(t, cid, drive_link)
        except Exception:
            pass
    ...
    sub_entry = {
        ...
        "employee_id": submitter_eid,
        "primary_assignee": t.get("assignee_name"),
        "secondary_assignee": t_sec_name or None,
        "drive_link": drive_link,
        "notes": notes,
    ...
    _append_task_log(t, "submitted",
                     ...
                     actor_id=submitter_eid,
                     actor_type="employee",
                     note=notes or f"تسليم المخرجات لمدير الحساب بواسطة {submitter_name}",
                     details={"notes": notes, "drive_link": drive_link, "elapsed_minutes": mins, "submission_id": sub_entry["id"], "submitted_by": submitter_name})
    save_one_task(t, cid)
    
    # Ping the responsible account manager(s) via Telegram with complete delivery details
    if t_sec_name:
        emp_line = f" الموظفين (عمل مشترك): {t.get('assignee_name','')} + {t_sec_name}\n سلمه: {submitter_name}"
    else:
        emp_line = f" الموظف: {t.get('assignee_name','')}"
    _notify_client_am(cid,
        ...
        task=t)
    return jsonify({"ok": True, "task": t, "minutes": mins, "drive_link": drive_link, "submitted_by": submitter_name})
...
def api_my_task_request_return(task_id):
    ...
    now_iso = datetime.now(timezone.utc).isoformat()
    t["status"] = "In Progress"
    t["returned_to_employee_at"] = now_iso
...
def api_tasks_set_drive_link(task_id):
    ...
    save_one_task(t, cid)
    try:
        _ensure_task_deliverable_in_client_drive(t, cid, link, filename=fname)
    except Exception:
        pass
...
def api_task_review(task_id):
    ...
    sec_emp = _sheet_emp(t.get("secondary_employee_id"))
    sec_tg = str(sec_emp.get("telegram_id", "")).replace(".0", "").strip() if t.get("secondary_employee_id") else ""
    ...
    if action in ("reject", "return"):
        ...
        if ntg:
            send_telegram_bot_notification(ntg,
                f" <b>مهمة وصلتلك</b>\n {nxt.get('name','')}\n {t.get('title','')}\n"
                f" {_client_name(cid)}\n📅 موعد التسليم: {t.get('delivery_deadline') or t.get('publish_date','-')}\nابدأ من البوابة 🚀")
    else:
        ...
        if sec_tg and sec_tg != cur_tg:
            send_telegram_bot_notification(sec_tg,
                f"🎉 <b>تم اعتماد مهمتك بنجاح (عمل مشترك) ✅</b>\n📌 {t.get('title','')}\n🏢 {_client_name(cid)}\n"
                + (f"💬 {review_note}" if review_note else "تسلم إيدك، شغل ممتاز!"))
    save_one_task(t, cid)
    return jsonify({"ok": True, "task": t})


@app.route("/api/tasks/bulk-review", methods=["POST", "PUT"])
@require_manager
def api_tasks_bulk_review():
    """Bulk review and finalize submitted tasks by Account Manager."""
    sync_from_supabase()
    data = request.get_json() or {}
    task_ids = data.get("task_ids") or []
    action = (data.get("action") or "finalize").lower()
    review_note = (data.get("note") or "اعتماد جماعي بواسطة مدير الحسابات").strip()
    if not task_ids or not isinstance(task_ids, list):
        return jsonify({"ok": False, "error": "قائمة المهام فارغة"}), 400
    ...
    for tid in task_ids:
        ...
        if not t:
            failed.append({"task_id": tid, "reason": "not_found"})
            continue
        if not can_see_client(cid):
            failed.append({"task_id": tid, "reason": "forbidden"})
            continue
        ...
        if action in ("finalize", "approve"):
            ...
            _append_task_log(t, "reviewed_approved",
                             ...
                             actor_type="account_manager",
                             note=review_note,
                             details={"review_note": review_note, "bulk": True})
            cur_emp = _sheet_emp(t.get("assigned_employee_id"))
            cur_tg = str(cur_emp.get("telegram_id", "")).replace(".0", "").strip()
            if cur_tg:
                try:
...
def _task_card_text(t, cid):
    ...
    st_ar = {
        ...
        "Awaiting AM Review": "تم التسليم / بانتظار المراجعة 📤",
        "Submitted / In Review": "تم التسليم / بانتظار المراجعة 📤",
    ...
    sec_name = (t.get("secondary_assignee_name") or "").strip()
    sec_eid = str(t.get("secondary_employee_id") or "")
    if sec_eid and (not sec_name or sec_name == sec_eid):
        sec_name = _sheet_emp(sec_eid).get("name", sec_name) or sec_eid
    post_num = _resolve_post_number(t)
    deadline = t.get("delivery_deadline") or t.get("publish_date") or "-"
    assignee_line = f"👥 فريق العمل: <b>{name}</b> + <b>{sec_name}</b>" if sec_name else f"👤 المنفذ: {name}"
    lines = [
        f"🎯 <b>بوست #{post_num}</b> (<code>{t.get('task_id','')}</code>)",
        f"📅 <b>موعد التسليم: {deadline}</b> ⏰",
        assignee_line,
        f"🏢 {_client_name(cid)}",
        f"📌 <b>{t.get('title','')}</b>",
    ]
    ...
    if vis and vis != t.get("title") and vis != t.get("caption"):
        if len(vis) > 800:
            clean_vis = vis[:800].rsplit(' ', 1)[0] + "\n... (باقي التوجيهات بالبورتال)"
        else:
            clean_vis = vis
    ...
    if c_text:
        if len(c_text) > 1800:
            clean_c = c_text[:1800].rsplit(' ', 1)[0] + "\n... (باقي الكابشن متاح كاملاً بالبورتال)"
        else:
            clean_c = c_text
        if clean_c != t.get("title"):
...
def _task_card_buttons(t):
    ...
    elif st in ("Awaiting AM Review", "Submitted / In Review", "Submitted"):
        rows.append([{"text": "🔍 تم التسليم وبانتظار مراجعة المدير", "callback_data": "noop"}])
    elif st == "Completed":
        rows.append([{"text": "️ مكتملة ومعتمدة ✅", "callback_data": "noop"}])

    # Direct portal link button to view 100% full content and copy caption
    rows.append([{"text": "🌐 فتح البورتال / التفاصيل الكاملة ↗", "url": "https://metaaimoderator.vercel.app/#myportal"}])
...
def send_task_to_employee(t, cid):
    """Send an interactive task card to the assigned employee's Telegram (and co-assignee if present)."""
    tg = _emp_chat(t.get("assigned_employee_id"))
    sec_tg = _emp_chat(t.get("secondary_employee_id"))
    sent = False
    if tg:
        sent = bool(_tasks_send(tg, _task_card_text(t, cid), inline=_task_card_buttons(t))) or sent
    if sec_tg and str(sec_tg).strip() != str(tg).strip():
        sent = bool(_tasks_send(sec_tg, _task_card_text(t, cid), inline=_task_card_buttons(t))) or sent
    return sent
...
def _tasks_handle_callback(cbq):
    ...
    if data.startswith("ts_") or data.startswith("tb_") or data.startswith("tr_"):
        ...
        if not t:
            ...
            return
        # only the assignee or co-assignee may act
        assignee_tg = _emp_chat(t.get("assigned_employee_id"))
        sec_tg = _emp_chat(t.get("secondary_employee_id"))
        is_primary = bool(assignee_tg and presser == str(assignee_tg).strip())
        is_secondary = bool(sec_tg and presser == str(sec_tg).strip())
        if not (is_primary or is_secondary):
            _tasks_answer(cb_id, "دي مش مهمتك")
            return

        actor_name = (t.get("secondary_assignee_name") or "شريك العمل") if is_secondary else (t.get("assignee_name") or "الموظف")
        actor_id = t.get("secondary_employee_id") if is_secondary else t.get("assigned_employee_id")
...
def _tasks_do_submit(t, cid, note, presser_tg=None):
    ...
    sec_tg = _emp_chat(t.get("secondary_employee_id"))
    is_sec = bool(presser_tg and sec_tg and str(presser_tg).strip() == str(sec_tg).strip())
    sub_name = (t.get("secondary_assignee_name") or "شريك العمل") if is_sec else (t.get("assignee_name") or "الموظف")
    sub_eid = t.get("secondary_employee_id") if is_sec else t.get("assigned_employee_id")
    t["submitted_by"] = sub_name
    t["submitted_by_id"] = sub_eid
    ...
    collab_str = f"👥 فريق العمل: {t.get('assignee_name','')} + {t.get('secondary_assignee_name','')}\n👤 سلمه: <b>{sub_name}</b>\n" if t.get("secondary_employee_id") else f"👤 {t.get('assignee_name','')}\n"
    _notify_client_am(cid,
        f"📤 <b>مهمة جاهزة للمراجعة</b>\n{collab_str}📌 {t.get('title','')}\n"
        f"🏢 {_client_name(cid)}\n⏱️ {mins} دقيقة\n✍️ ملاحظات: {note or '—'}\n" +
        (f"📁 رابط الدرايف: {drive_link}" if drive_link else ""),
        task=t)
...
def telegram_tasks_webhook():
    ...
    try:
        ...
        if not tid and uploaded_drive_link:
            ...
            if emp_eid:
                all_t = _all_tasks_db()
                active_t = next((x for x in all_t if (str(x.get("assigned_employee_id")) == emp_eid or str(x.get("secondary_employee_id")) == emp_eid) and x.get("status") in ("In Progress", "Assigned")), None)
                if active_t:
        ...
        if tid:
            ...
            if t and t.get("status") in ("In Progress", "Assigned", "Pending AM Approval"):
                ...
                _tasks_do_submit(t, cid, note, presser_tg=tg_id)
