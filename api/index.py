...
def _save_and_distribute_plan_to_drive(client_id, client_name, plan_name, posts, am_id=None, am_name=None, creator_id=None, creator_name=None):
    ...
    try:
        ...
        try:
            ...
            c_name = creator_name or "فريق المحتوى"
        ...
        # Collect all assigned employees' plan folders
        emp_fids = []
        seen_eids = {amid, c_eid}
        for p in (posts or []):
...
def _ensure_task_plan_in_employee_drive(task, cid):
    """When a task is assigned or reassigned to an employee, ensure the employee has the
    plan folder and plan DOCX shortcut in their Drive."""
    try:
        if not isinstance(task, dict):
            return
        eid = str(task.get("assigned_employee_id") or "").strip()
        ename = str(task.get("assignee_name") or "").strip()
        if not eid or eid.lower() in ("none", "unassigned", "null"):
            return
        ...
        if target_doc_id:
            ...
            _save_and_distribute_plan_to_drive(cid, cname, pname, plan_tasks)
    except Exception as _e:
