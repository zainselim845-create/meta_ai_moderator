...
def _gsheet_tasks_save_all(tasks_list):
    """Persist all tasks to the Tasks tab in the Google Spreadsheet."""
...
def api_plans_delete():
    ...
    if not deleted_tasks:
        return jsonify({"success": True, "message": "الخطة غير موجودة أو تم حذفها بالفعل", "deleted_count": 0})
    ...
    return jsonify({
        "success": True,
        "ok": True,
        "message": f"تم حذف الخطة «{plan_name}» وجميع مهامها ({len(deleted_tasks)} مهمة) بنجاح 🗑️",
