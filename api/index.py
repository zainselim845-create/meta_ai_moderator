    ...
    if action in ("reject", "return"):
        ...
        if ntg:
            send_telegram_bot_notification(ntg,
                f" <b>مهمة وصلتلك</b>\n {nxt.get('name','')}\n {t.get('title','')}\n"
                f" {_client_name(cid)}\n📅 موعد النشر: {t.get('publish_date') or t.get('delivery_deadline','-')}\nابدأ من البوابة 🚀")
    else:
...
def _task_card_text(t, cid):
    ...
    deadline = t.get("publish_date") or t.get("delivery_deadline") or "-"
    lines.append(f"📅 موعد النشر: <b>{deadline}</b>")
