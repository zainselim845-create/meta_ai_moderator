...
CANONICAL_CLIENT_ALIASES = {
    ...
    "cli_hayat_dental_center_1788685057": [
        ...
        "د شاهنده", "دكتورة شاهندة", "دكتورة شاهنده", "د/ شاهنده", "د. شاهنده", "د/ شاهندة", "د. شاهندة", "شاهنده", "شاهندة"
    ],
...
def push_setting(key, value):
    ...
    if key == "meta_ai_clients":
        if not value or not isinstance(value, list) or len(value) < 3:
            ...
            return
        # Ensure bogus clients never get pushed
        value = [c for c in value if isinstance(c, dict) and "إبداع" not in str(c.get("name","")) and "إبداع" not in str(c.get("company","")) and "عميل عام" not in str(c.get("name",""))]
...
def _send_due_task_reminders(force=False):
    ...
    for t in all_tasks:
        ...
        if due == tomorrow and eid and st in ("Assigned", "In Progress"):
            emp_due_tomorrow.setdefault(eid, []).append(t)
            
        # 2. Employee: Due Today or Overdue (مستحقة اليوم أو متأخرة)
        if due <= today and eid and st in ("Assigned", "In Progress"):
    ...
    for am_key, tasks in am_due_today.items():
        ...
        for t in tasks[:12]:
            ...
            st_text = "قيد المراجعة ⏳" if t.get("status") == "Awaiting AM Review" else ("جاري العمل ⏱️" if t.get("status") == "In Progress" else "بانتظار البدء")
...
def _ensure_client_record(client_input, am_id=None):
    ...
    if not target:
        ...
        elif any(k in norm_input or k in client_str_low for k in ["شاهنده", "شاهندة", "shahinda", "حياة", "hayat", "اسنان", "أسنان", "فينير", "فينيرز", "dental"]):
...
def api_tasks_ingest_plan():
    try:
        ...
        # If frontend passed Domya or default, but the file explicitly names another doctor/brand, respect the document!
        if cid_param_clean and cid_param_clean not in ("all", "__all__"):
            if cid_param_clean == "client_100821894800009" and detected_brand_input and detected_brand_input != "client_100821894800009":
                client_input = detected_brand_input
            else:
                client_input = cid_param_clean
        elif cname_param_clean and cname_param_clean not in ("عميل عام", "العميل", "None", "null"):
            if cname_param_clean == "Domya Marketing Agency" and detected_brand_input and detected_brand_input != "client_100821894800009":
                client_input = detected_brand_input
            else:
        ...
        if global_assigned_name and not global_assigned_eid:
        ...
        elif global_assigned_eid and not global_assigned_name:
            _reid, _rname = _resolve_creator_employee(global_assigned_eid)
            global_assigned_name = _rname or global_assigned_eid
    ...
    if is_adm and target_eid and target_eid not in ("all", "me"):
        ...
        emp_tasks = [
            ...
            if str(t.get("assigned_employee_id") or "").strip() == target_eid or 
               target_eid.lower() in str(t.get("assignee_name") or "").lower()
        ]
