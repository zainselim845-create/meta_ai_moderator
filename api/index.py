...
def _send_due_task_reminders(force=False):
    ...
    for t in all_tasks:
        ...
        if due <= today:
            am_key = am_id or am_name or "AM-2072-9827"
...
def api_my_tasks():
    ...
    if is_adm and target_eid and target_eid != "all":
        all_tasks = _all_tasks_db()
        emp_tasks = [
    ...
    # If admin and has no personal assigned tasks, default to showing all tasks so admin view is never empty
    if is_adm and not mine:
        mine = list(all_tasks)
            
    mine.sort(key=_natural_task_sort_key)
    return jsonify({"tasks": mine, "employee_id": eid or "all", "is_admin": is_adm})
