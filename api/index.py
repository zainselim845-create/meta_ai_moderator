...
def _clean_task_for_gsheet(t):
    ...
    if "content_data" in clean and isinstance(clean["content_data"], dict):
        cd = dict(clean["content_data"])
        if "reference_images" in cd and isinstance(cd["reference_images"], list):
            cd["reference_images"] = [u for u in cd["reference_images"] if not str(u).startswith("data:image")]
        clean["content_data"] = cd
    if "graphic_data" in clean and isinstance(clean["graphic_data"], dict):
        gd = dict(clean["graphic_data"])
        if "reference_images" in gd and isinstance(gd["reference_images"], list):
            gd["reference_images"] = [u for u in gd["reference_images"] if not str(u).startswith("data:image")]
        clean["graphic_data"] = gd
    raw_s = json.dumps(clean, ensure_ascii=False)
    if len(raw_s) > 35000:
        raw_s = raw_s[:35000]
    return raw_s
