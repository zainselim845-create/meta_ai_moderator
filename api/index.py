...
def api_tasks_ingest_plan():
    ...
    for line in lines:
        if line.startswith("#") or len(line) < 3:
            continue
        # pull any reference links / image URLs out of the line and attach to the task
        found_urls = _url_re.findall(line)
        line_clean = _url_re.sub("", line).strip(" |")
        parts = [p.strip() for p in line_clean.split("|") if p.strip()] if "|" in line_clean else [line_clean or line]
        title = parts[0]
        ...
        # split urls into image refs (drive / image extensions) vs generic reference links
        image_urls, ref_links = [], []
        for u in found_urls:
            low = u.lower()
            if ("drive.google" in low) or any(low.split("?")[0].endswith(ext) for ext in (".jpg", ".jpeg", ".png", ".webp", ".gif")):
                image_urls.append(u)
            else:
                ref_links.append(u)
        ...
        new_task = {
            ...
            "content_data": {"post_type": post_type, "tag_line": title, "reference_images": image_urls},
            "graphic_data": {"idea": desc, "reference_images": image_urls, "reference_links": ref_links},
            "created_at": datetime.now(timezone.utc).isoformat()
        }
        tasks.insert(0, new_task)
        counter += 1
