...
def api_tasks():
    ...
    if is_admin():
        ...
        # Account Manager sees ONLY their assigned clients and tasks
        my_cids = assigned_client_ids()
        my_eid = _my_employee_id()
        my_name = (current_user_rec().get("name") or current_username()).strip().lower()
        if req_cid and req_cid != "all" and req_cid in my_cids:
            raw_tasks = get_client_tasks(req_cid)
        else:
        ...
        my_cids = assigned_client_ids()
        if req_cid and req_cid != "all" and (not my_cids or req_cid in my_cids):
            raw_tasks = get_client_tasks(req_cid)
        else:
            all_db = _all_tasks_db()
            raw_tasks = [t for t in all_db if not my_cids or t.get("client_id") in my_cids]
    else:
