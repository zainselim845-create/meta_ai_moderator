...
def _resolve_creator_employee(creator_input):
    """Matches a creator input (name, substring, or employee ID) to an Agency employee record.
    Returns (employee_id, employee_name)."""
    ...
    clean_inp = re.sub(r'^(?:content\s*creator|copywriter|writer|الكونتنت\s*كريتور|كاتب\s*المحتوى|صانع\s*المحتوى|كاتب|المعد|بواسطة|إعداد|by)\s*[:=\-]*\s*', '', inp, flags=re.I).strip()
    clean_low = clean_inp.lower()

    # 1. Check known aliases map first
    known_aliases = {
    ...
    for alias, (alias_id, alias_name) in known_aliases.items():
        if alias in clean_low:
            return (alias_id, alias_name)

    # 2. Query agency employees list
    try:
        cfg = hr_config()
...
def api_tasks_ingest_plan():
    try:
        ...
        creator_input = str(creator_param or "").strip()
        if not creator_input or creator_input.lower() in ("auto", "none", "null"):
